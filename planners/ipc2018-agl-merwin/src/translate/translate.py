...
COMPILE_AWAY_CONDITIONAL_EFFECTS = True
DUMP_TASK = False
...
def translate_strips_operator(operator, dictionary, ranges, mutex_dict, mutex_ranges, implied_facts):
    ...
    for comp_operator in compiled_operators(operator):
        ...
        for condition in conditions:
            ops = translate_strips_operator_aux(comp_operator, dictionary, ranges,
                                               mutex_dict, mutex_ranges,
                                               implied_facts, condition)
            sas_operators.extend(ops)
...
def translate_strips_operator_aux(operator, dictionary, ranges, mutex_dict,
    ...
    sas_ops = []
    if COMPILE_AWAY_CONDITIONAL_EFFECTS: # WARNING: destroys effects_by_variable and condition
        def split(index, deletes_for_variables, condition, effects_by_variable,
                  sas_operators):
            if index == len(deletes_for_variables):
                op = build_sas_operator(operator.name, dict(condition),
                                        effects_by_variable, operator.cost, ranges,
                                        implied_facts)
                if op is not None:
                    sas_operators.append(op)
                return

            var, include = deletes_for_variables[index]
            none_of_those =  ranges[var] - 1
            for val, inc in enumerate(include):
                if inc:
                    effects_by_variable[var][none_of_those] = [{}]
                condition[var] = val
        ...
        for var in effects_by_variable:
            ...
            if none_of_those in effects_by_variable[var]:
                if ranges[var] == 2: 
                    ...
                    # action application var is always none_of_those and we
                    # can simply drop the condition
                    assert(len(effects_by_variable[var]) == 1)
                    effects_by_variable[var][none_of_those] = [{}]
                    continue
                eff_condition = effects_by_variable[var][none_of_those]
                include = [False] * ranges[var]
                for cond in eff_condition:
                    if not cond:
                        effects_by_variable[var][none_of_those] = [cond]
                        break
                    assert(len(cond) == 1) # only guard on del effect
                    include[cond[var]] = True
                else:
                    print(eff_condition)
                    del(effects_by_variable[var][none_of_those])
                    deletes_for_variables.append((var, include))
        if deletes_for_variables:
            print(operator.name)
            print(deletes_for_variables)
            split(0, deletes_for_variables, condition, effects_by_variable, sas_ops)
            return sas_ops

    op = build_sas_operator(operator.name, condition, effects_by_variable,
