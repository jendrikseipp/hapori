...
const MEDDLY::unary_opname* SWAPTOPRIMEDVAR;

class swapToPrimedVar: public MEDDLY::unary_operation {
  public:
  swapToPrimedVar(const MEDDLY::unary_opname* code, MEDDLY::expert_forest* arg,
  ...
  MEDDLY::node_handle &rp) {
    ...
    if (resultLevel % 2 == 0 && resultLevel > 0) {
      resultLevel -= 1;
    }
...
class swapToPrimedVar_opname: public MEDDLY::unary_opname {
  public:

  swapToPrimedVar_opname() :
  unary_opname("SwapToPrimedVar") {
  }

  MEDDLY::unary_operation* buildOperation(MEDDLY::expert_forest* arg1,
  MEDDLY::expert_forest* res) const {
    return new swapToPrimedVar(this, arg1, res);
  }
};

void initializeSwapToPrimedVar() {
  SWAPTOPRIMEDVAR = new swapToPrimedVar_opname;
}
