  ...
  int& cev, node_handle& c)
{
  ...
  for (int i=0; i<resultSize; i++) {
    int ev;
    node_handle ed;
    // David added: check for inf
    int a_val = aev + A->ei(i);
    if ((a_val < 0 && aev >= 0 && A->ei(i) >= 0)
            || (aev == std::numeric_limits<int>::max()
            && A->ei(i) == std::numeric_limits<int>::max()))
        a_val = std::numeric_limits<int>::max();
    int b_val = bev + B->ei(i);
    if ((b_val < 0 && bev >= 0 && B->ei(i) >= 0)
            || (bev == std::numeric_limits<int>::max()
            && B->ei(i) == std::numeric_limits<int>::max())) 
        b_val = std::numeric_limits<int>::max();
    compute(a_val, A->d(i),
            b_val, B->d(i),
            ev, ed);
    nb->d_ref(i) = ed;
