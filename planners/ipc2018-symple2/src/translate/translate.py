...
def translate_strips_operator(operator, dictionary, ranges, mutex_dict,
    ...
    # hack by gabi which is really problematic
    """def compiled_operators(operator):
        ...
        for i in range(2**(len(conditional_effects))):
            ...
            for cond in all_conditions:
                new_effects = [([], e) for _, e in effects if e not in cond]
                if new_effects:
                    yield pddl.PropositionalAction(operator.name, cond, new_effects,
                                                   operator.cost)
...
def get_is_a(task):
    # Create hierarchical object type dict
    is_a = {}
    #print([t.name for t in task.types])
    for cur_type in task.types:
        #if cur_type.name == "object":
        #    continue
        is_a[cur_type] = [t for t in task.types if t.name == cur_type.basetype_name]
        if len(is_a[cur_type]) != 1 and cur_type.name != "object":
            return {}
        elif cur_type.name != "object":
            is_a[cur_type] = is_a[cur_type][0]
    return is_a

def subtype_of(ty, is_a, task):
    types = [ty.name]
    key = ty
    while key.basetype_name is not None:
        #print(is_a[key])
        types.append(is_a[key].name)
        key = is_a[key]
    return types


def getFilteredType(set_of_types, is_a, task):
    # Map type strings to actual type object
    sort_types = []
    for t in task.types:
        if t.name in set_of_types:
            sort_types.append(t)
    #print(sort_types)

    final_sort_types = []
    # remove subtypes
    for cur_type in sort_types:
        if is_a[cur_type] not in sort_types:
            final_sort_types.append(cur_type)
    return final_sort_types


def add_ordering_to_task(list_of_types, obj_to_types, task):
    for cur_type in list_of_types:
        name = "less_than_" + cur_type.name
        # two arguments
        arguments = []
        arguments.append(pddl.TypedObject("?x", cur_type))
        arguments.append(pddl.TypedObject("?y", cur_type))
        pred = pddl.Predicate(name, arguments)
        task.predicates.append(pred)

        # collect all elements
        object_with_type = [o for o in task.objects if str(cur_type) in obj_to_types[o.type]]
        # add lessThan for all objects of the current type
        for i in range(len(object_with_type)):
            for j in range(i, len(object_with_type)):
                atom = pddl.Atom(pred.name, [object_with_type[i].name, object_with_type[j].name])
                task.init.append(atom)


def add_ordering_to_action(action, names_to_types, types_to_order, types_to_subtypes, tr):
    params_to_type = {}
    names_to_params = {}
    for p in action.parameters:
        params_to_type[p.name] = names_to_types[p.type]
        names_to_params[p.name] = p
    pre_atoms = list(action.precondition.parts)
    for r in tr:
        # we search for the correct typ
        possible_types = types_to_subtypes[params_to_type[r[0]].name]
        sort_type_names = [y.name for y in types_to_order]
        name = "less_than_" + [x for x in possible_types if x in sort_type_names][0]
        # get the predicate from the task
        atom = pddl.Atom(name, [r[0], r[1]])
        #print(atom)
        pre_atoms.append(atom)
    action.precondition.parts = tuple(pre_atoms)

def sym_reduction(task):
    object_types = set([])
    transition_relations = {}
    for action in task.actions:
        symFinder = symReduction.SymFinder()
        tr = symFinder.find_symmetries(action)
        if len(tr) == 0:
            continue
        #print(action.name)
        #print(tr)
        transition_relations[action] = tr
        # Collect object we need to order
        objects = [x[0] for x in tr] + [x[1] for x in tr]
        objs_set = set(objects)
        for p in action.parameters:
            if p.name in objs_set:
                object_types.add(p.type)

    print("Prunable PDDL-operators: " + str(len(transition_relations)))
    if len(transition_relations) == 0:
        return
    is_a = get_is_a(task)
    #print(is_a)
    if len(is_a) == 0:
        return
    types_to_order = getFilteredType(object_types, is_a, task)

    # get types of all objects
    types_to_subtypes = {}
    for t in task.types:
        types_to_subtypes[t.name] = subtype_of(t, is_a, task)

    add_ordering_to_task(types_to_order, types_to_subtypes, task)

    # add to action, before we need to get types from their string...
    names_to_types = {}
    for t in task.types:
        names_to_types[t.name] = t

    for action in transition_relations:
        add_ordering_to_action(action, names_to_types, types_to_order, types_to_subtypes, transition_relations[action])
        #action.dump()
...
def main():
    ...
    # TODO(speckd): add sym reduction input parameter
    sym_reduction(task)
    #task.dump()
    ...
    #with timers.timing("Compile Cond. Effects"):
    	#compileCondEffs.compileCondEffAway("output.sas", "output.sas")
    print("Done! %s" % timer)
