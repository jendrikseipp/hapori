...
const Operator *parent_op) {
  ...
  if (domain_has_sdac()) {
    ...
    + get_adjusted_action_cost(*parent_op, parent_node.get_state(), cost_type);
    info.real_g = parent_node.info.real_g
    + parent_op->get_cost(parent_node.get_state());
  } else {
    info.g = parent_node.info.g
  ...
  for (;;) {
    ...
    if (domain_has_sdac()) {
      ...
      for (int i = 0; i < g_variable_domain.size(); i++) {
        vals.push_back(static_cast<int>(info.parent_state[i]));
      }
      Operator* dummy = new Operator(op->get_name(), op->get_cost(State(vals)));
