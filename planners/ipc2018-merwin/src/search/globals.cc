...
//Michael: added for gluing.
const set<pair<int, int> >& get_inconsistent_facts(int var, int val) {
    return g_inconsistent_facts[var][val];
}
...
// Michael: added for pre-merge
std::vector<std::vector<std::pair<int, int> > > g_invariant_groups;
...
std::string get_variable_name_and_domain(int var) {
	std::stringstream ss;

//	ss << "[" <<var << "] " << g_variable_name[var] << ": " ;
	ss << "{ " ;
	ss << g_fact_names[var][0];

	for (int f = 1; f < g_fact_names[var].size(); f++)
		ss << ", "<< g_fact_names[var][f];
