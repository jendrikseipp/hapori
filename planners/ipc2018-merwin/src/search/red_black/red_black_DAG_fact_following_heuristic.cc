...
RedBlackDAGFactFollowingHeuristic::RedBlackDAGFactFollowingHeuristic(
        const Options &opts) :
        FFHeuristic(opts), applicability_status(false), num_invertible_vars(0), shortest_paths_calculated(
                false), use_black_dag(false), ignore_invertibility(
                opts.get<bool>("ignore_invertibility")), preferred_type(
                PreferredOpsType(opts.get_enum("prefs"))), paint_roots_black(
                opts.get<bool>("paint_roots_black")), applicable_paths_first(
                opts.get<bool>("applicable_paths_first")), use_connected(
                opts.get<bool>("use_connected")), next_red_action_test(
                opts.get<bool>("next_red_action_test")), extract_plan_no_blacks(
                opts.get<bool>("extract_plan_no_blacks")) {
...
void RedBlackDAGFactFollowingHeuristic::initialize() {
    ...
    if (!g_axioms.empty()) {
        cerr
                << "error: Red-Black heuristic implementation does not support axioms."
                << endl;
    ...
    for (size_t ind = 0; ind < black_indices.size(); ++ind) {
        ...
        if (pred.size() > 0) {
            ...
            for (size_t i = 0; i < pred.size(); ++i) {
                int pred_var = pred[i];
                if (black_vars[pred_var]
                        || connectivity_status[pred_var]
                                != ALL_PAIRS_CONNECTED) {
    ...
    cout
            << "Finished initializing Red-Black Relaxation heuristic at time step [t="
            << g_timer << "]" << endl;
}
...
void RedBlackDAGFactFollowingHeuristic::dump_options() const {
    ...
    if (ignore_invertibility) {
        cout
                << "Warning! Running with ignore_invertibility option ON is not safe - may return DEAD_END for non dead end states. "
                << endl;
    }
    cout
    ...
    if (preferred_type == RELAXED) {
        ...
        cout << "from relaxed plan and applicable black changing actions"
                << endl;
    } else if (preferred_type == PRECS_UNCHANGED) {
        cout << "by unchanged preconditions along red-black plan actions"
                << endl;
    } else {
...
void RedBlackDAGFactFollowingHeuristic::prepare_DTGs_for_invertibility_check() {
    ...
    cout << "Adding " << g_operators.size() << " operators to extended DTGs"
            << endl;
    ...
    for (int op_no = 0; op_no < num_ops; ++op_no) {
        ...
        for (size_t i = 0; i < pre_post.size(); ++i) {
            ...
            for (int value = pre_value_min; value < pre_value_max; ++value) {
                get_dtg(var)->add_operator_from_to(value, post_value,
                        rb_sas_op);
            }
        }
    }
...
void RedBlackDAGFactFollowingHeuristic::free_mem() {
    for (size_t var = 0; var < dtgs_by_transition.size(); ++var) {
        delete get_dtg(var);
    }
...
void RedBlackDAGFactFollowingHeuristic::set_black_variables() {
    ...
    cout << "Total number of invertible variables is " << num_invertible_vars
            << endl;
    ...
    cout << "Invertible variables connection status: "
            << are_black_variables_connected() << endl;
    ...
    cout << "Invertible variables without leafs connection status: "
            << are_black_variables_connected() << endl;
    ...
    if (num_blacks > 1 && paint_roots_black) {
        ...
        for (int i = 0; i < num_vars; ++i) {
            ...
            if (get_cg_predecessors(i).size() == 0) {
                cout << "Forced painting root " << g_variable_name[i]
                        << " black. As a result, painting red the following invertible variables: ";
                const vector<int>& succ = get_cg_successors(i);
...
void RedBlackDAGFactFollowingHeuristic::set_red_black_indices() {
    ...
    for (size_t i = 0; i < black_vars.size(); ++i) {
        if (!black_vars[i]) {
...
void RedBlackDAGFactFollowingHeuristic::print_statistics() const {
    ...
    int count_all_pairs_connected = 0, count_all_connected_to_goal = 0,
            count_almost_root = 0;
    ...
    for (size_t op_no = 0; op_no < g_operators.size(); ++op_no) {
        ...
        if (pre_post.size() < 2)
            continue;
        for (size_t i = 0; i < pre_post.size() - 1; ++i) {
            int var = pre_post[i].var;
...
//    cout << "---------------------------------------------------------------------------------------" << endl;
    int num_vars = g_variable_domain.size();
    for (int var_no = 0; var_no < num_vars; ++var_no) {
    ...
    cout << "Total number of variables with all pairs of values connected is "
            << count_all_pairs_connected << endl;
    cout << "Total number of variables with all values connected to goal is "
            << count_all_connected_to_goal << endl;
    cout
            << "Total number of black variables with strongly connected parents only is "
            << count_almost_root << endl;
    cout << "Maximal number of side effects for black variable is "
            << max_side_effect_for_black << endl;
    cout
            << "---------------------------------------------------------------------------------------"
            << endl;
    if (count_black == 0) {
...
void RedBlackDAGFactFollowingHeuristic::paint_red_by_vertex_cover(
        vector<int>& order, int* elements) {
...
void RedBlackDAGFactFollowingHeuristic::get_cg_neighbours(vector<int> &neigh,
        int node) const {
    ...
    set_union(pred.begin(), pred.end(), succ.begin(), succ.end(),
            back_inserter(neigh));
...
void RedBlackDAGFactFollowingHeuristic::prepare_operators_for_counting_achieved_preconditions() {
    ...
    for (size_t op_no = 0; op_no < g_operators.size(); ++op_no) {
        const partial_assignment& red_pre =
                get_rb_sas_operator(op_no)->get_red_precondition();
        for (partial_assignment::iterator it = red_pre.begin();
                it != red_pre.end(); it++) {
    ...
    for (int op_no = 0; op_no < num_ops; ++op_no) {
        ...
        for (size_t i = 0; i < pre_post.size(); ++i) {
            ...
            for (int value = pre_value_min; value < pre_value_max; ++value) {
                ...
                get_dtg(var)->add_edge_to_complete_forward_graph(value,
                        post_value, op_no, op_cost, is_root);
            }
...
void RedBlackDAGFactFollowingHeuristic::precalculate_shortest_paths_for_var(
        size_t var) {
    if (shortest_paths_calculated)
        return;

    if (!black_vars[var] && use_connected) {
        cout << "Storing shortest paths and costs for connected red variable "
                << var << endl;
...
bool RedBlackDAGFactFollowingHeuristic::is_already_DAG(
        const vector<bool>& blacks) const {
...
int RedBlackDAGFactFollowingHeuristic::get_best_index(vector<int>& order,
        int* elements) {
...
int RedBlackDAGFactFollowingHeuristic::get_index_of_leftmost_nonzero(
        vector<int>& order, int* elements) const {
...
void RedBlackDAGFactFollowingHeuristic::get_relaxed_plan(const State &state,
        Proposition *goal) {
...
int RedBlackDAGFactFollowingHeuristic::get_semi_relaxed_plan_cost(
        const State &state) {
    ...
    while (true) {
        ...
        const Operator* op = &g_operators[op_no];
        ActionApplicationResult app_status = apply_action_to_semi_relaxed_state(
                op_no);
...
void RedBlackDAGFactFollowingHeuristic::set_new_marks_for_state(
        const State &state) {
    ...
    for (size_t ind = 0; ind < red_indices.size(); ++ind) {
        ...
        if (0 < get_dtg(var)->num_sufficient_unachieved()) {
            red_sufficient_unachieved_iterators[var] =
                    red_sufficient_unachieved.insert(
                            red_sufficient_unachieved.end(), var);
        }
    }
...
int RedBlackDAGFactFollowingHeuristic::add_red_black_plan_suffix(
        const State &state, int h_val) {
...
void RedBlackDAGFactFollowingHeuristic::mark_red_precondition(int var,
        int val) {
...
int RedBlackDAGFactFollowingHeuristic::get_next_action(
        bool skip_black_pre_may_delete_red_sufficient_achieved) {
    ...
    for (list<int>::const_iterator it = red_sufficient_unachieved.begin();
            it != red_sufficient_unachieved.end(); ++it) {
        int var = *it;
        ...
        const list<int>& sufficient_unachieved =
                get_dtg(var)->get_sufficient_unachieved();
        for (list<int>::const_iterator it2 = sufficient_unachieved.begin();
                it2 != sufficient_unachieved.end(); ++it2) {
            int val = *it2;
#ifdef DEBUG_RED_BLACK
            ...
            for (int o = 0; o < max_num_ops; ++o) {
                ...
                if (skip_black_pre_may_delete_red_sufficient_achieved
                        && achieving_black_pre_may_delete_achieved_red_sufficient(
                                op_no)) {
                    continue;
                }
                ...
                int cost =
                        get_operator_estimated_conflict_cost_black_reachability(
                                op_no);
    ...
    if (curr_min_op_id.size() == 0
            || curr_min_cost == numeric_limits<int>::max()) {
        if (skip_black_pre_may_delete_red_sufficient_achieved) {
#ifdef DEBUG_RED_BLACK
    ...
    int mixed_ind = -1;
    for (size_t ind = 0; ind < curr_min_op_id.size(); ++ind) {
...
bool RedBlackDAGFactFollowingHeuristic::achieving_black_pre_may_delete_achieved_red_sufficient(
        int op_no) const {
    const partial_assignment& black_pre =
            get_rb_sas_operator(op_no)->get_black_precondition();
    for (partial_assignment::iterator it = black_pre.begin();
            it != black_pre.end(); it++) {
        int var = (*it).first;
        int val = (*it).second;
        ...
        for (partial_assignment::iterator it2 = red_deletes.begin();
                it2 != red_deletes.end(); it2++) {
...
bool RedBlackDAGFactFollowingHeuristic::is_currently_mixed_effects(
        int op_no) const {
    ...
    const partial_assignment& black_eff =
            get_rb_sas_operator(op_no)->get_black_effect();
    for (partial_assignment::iterator it = black_eff.begin();
            it != black_eff.end(); it++) {
...
void RedBlackDAGFactFollowingHeuristic::add_path_for_var_from_to(int var,
        int from, int to, vector<int>& curr_sequence) {
...
const vector<int>& RedBlackDAGFactFollowingHeuristic::get_path_for_var_from_to(
    ...
    if (applicability_status && applicable_paths_first) {
        get_dtg(var)->set_only_current_transitions(true);
        const vector<int>& ops_to_add =
                get_dtg(var)->calculate_shortest_path_from_to(from, to);
...
const vector<int>& RedBlackDAGFactFollowingHeuristic::get_path_for_var(
        int var) {
    // First, trying to find a sequence based only on current values
    if (applicability_status && applicable_paths_first) {
        get_dtg(var)->set_only_current_transitions(true);
...
#endif
            const vector<int>& pre_ops =
                    get_dtg(pre_var)->calculate_shortest_path_from_to(from_val,
                            to_val);
#ifdef CRITICAL_RED_BLACK
            if (pre_ops.size() == 0) {
...
#endif
            current_applicable_sequence.insert(
                    current_applicable_sequence.end(), pre_ops.begin(),
                    pre_ops.end());
        }
...
int RedBlackDAGFactFollowingHeuristic::resolve_conflicts_DAG(
    ...
    int num_blacks = black_indices.size();
    for (int ind = num_blacks - 1; ind >= 0; ind--) {
        int var = black_indices[ind];
...
void RedBlackDAGFactFollowingHeuristic::set_op_as_preferred(const State &state,
        int op_no, bool relaxed) {
...
bool RedBlackDAGFactFollowingHeuristic::currently_op_prec_unchanged(
        int op_no) const {
    const partial_assignment& black_pre =
            get_rb_sas_operator(op_no)->get_black_precondition();
    for (partial_assignment::iterator it = black_pre.begin();
    ...
    const partial_assignment& red_pre =
            get_rb_sas_operator(op_no)->get_red_precondition();
    for (partial_assignment::iterator it = red_pre.begin(); it != red_pre.end();
            it++) {
        int var = (*it).first;
        int val = (*it).second;
...
void RedBlackDAGFactFollowingHeuristic::apply_action_to_current_state(
        int op_no) {
    // Check global applicability and apply
...
ActionApplicationResult RedBlackDAGFactFollowingHeuristic::apply_action_to_semi_relaxed_state(
        int op_no, bool check_applicability) {
    ...
    const partial_assignment& red_eff =
            get_rb_sas_operator(op_no)->get_red_effect();
    for (partial_assignment::iterator it = red_eff.begin(); it != red_eff.end();
            ++it) {
        int var = (*it).first;
        int val = (*it).second;
...
bool RedBlackDAGFactFollowingHeuristic::is_semi_relaxed_achieved(int var,
        int val) const {
    if (black_vars[var])
...
int RedBlackDAGFactFollowingHeuristic::compute_heuristic(const State &state) {
    ...
    int num_parallel_layers = parallel_relaxed_plan.size();
    for (int i = num_parallel_layers - 1; i >= 0; i--) {
    ...
    for (size_t i=0; i < suffix_plan.size(); ++i) {
        suffix_plan[i]->dump();
    }
    cout << "==========================================================================================" << endl;
#endif
...
void RedBlackDAGFactFollowingHeuristic::add_options_to_parser(
        OptionParser &parser) {
    ...
    parser.add_enum_option("prefs", pref_types, "relaxed",
            "preferred operators");
    ...
    // Extracting plan if no invertible variables are found and FF is used
...
void RedBlackDAGFactFollowingHeuristic::dump_current_semi_relaxed_state(
        bool dump_fact) const {
    int num_vars = g_variable_domain.size();
            ...
            for (size_t i = 0; i < pre_post.size(); ++i) {
                ...
                if (pre_value != -1) {
                    for (partial_assignment::iterator it = black_eff.begin();
                        ...
                        black_var_deletes[black_var].insert(
                                make_pair(var, pre_value));
                    }
                }
            }
        }
...
void RedBlackDAGFactFollowingHeuristic::keep_operators_by_effects() {
    ...
    for (size_t op_no = 0; op_no < g_operators.size(); ++op_no) {
        ...
        const partial_assignment& red_eff =
                get_rb_sas_operator(op_no)->get_red_effect();
        for (partial_assignment::iterator it = red_eff.begin();
                it != red_eff.end(); it++) {
...
void RedBlackDAGFactFollowingHeuristic::set_black_successors_by_ops() {
    ...
    for (size_t op_no = 0; op_no < g_operators.size(); ++op_no) {
        ...
        for (partial_assignment::iterator it = red_eff.begin();
            ...
            for (size_t i = 0; i < ops.size(); ++i) {
                const partial_assignment& black_eff = get_rb_sas_operator(
                        ops[i])->get_black_effect();
                for (partial_assignment::iterator it2 = black_eff.begin();
                        it2 != black_eff.end(); ++it2) {
                    black_variables.insert((*it2).first);
                }
            }
        }
        blacks_by_ops[op_no].insert(blacks_by_ops[op_no].begin(),
                black_variables.begin(), black_variables.end());
    }
}

int RedBlackDAGFactFollowingHeuristic::get_operator_estimated_conflict_cost_black_reachability(
        int op_no) const {
...
bool RedBlackDAGFactFollowingHeuristic::op_all_black_preconditions_reachable(int op_no) const {
    if (!use_black_dag)
        return true;
...
bool RedBlackDAGFactFollowingHeuristic::is_currently_applicable(const vector<int>& ops, bool skip_black) {
    ...
    if (ops.size() == 1) { // no need to copy the current state
        if (skip_black)
            return get_rb_sas_operator(op_no)->is_red_applicable(curr_state_buffer) && op_all_black_preconditions_reachable(op_no);
...
//            return false;
        ...
        if (skip_black) {
            if (!get_rb_sas_operator(op_no)->is_red_applicable(black_state_buffer) || !op_all_black_preconditions_reachable(op_no))
                return false;
        } else if (!get_rb_sas_operator(op_no)->is_applicable(black_state_buffer))
            return false;
    ...
    if (skip_black) {
        return get_rb_sas_operator(op_no)->is_red_applicable(black_state_buffer) && op_all_black_preconditions_reachable(op_no);
    }
    return get_rb_sas_operator(op_no)->is_applicable(black_state_buffer);
}
