...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        ...
        scriptOutput = os.path.abspath(sys.argv[1])
        scriptOutput = scriptOutput[:scriptOutput.rfind("/")]
        ...
        domain_wac  	 	  = scriptOutput + "/domain_without_action_costs.txt"
        problem_wac 	 	  = scriptOutput + "/problem_without_action_costs.txt"
        domain_wce  	 	  = scriptOutput + "/domain_wce.txt"
        problem_wce 	 	  = scriptOutput + "/facts_wce.txt"
        domain_ready  	 	  = scriptOutput + "/domain_ready.txt"
        problem_ready 	 	  = scriptOutput + "/problem_ready.txt"
        action_costs_domain_file  = scriptOutput + "/action_costs_from_domain_file.txt"
        action_costs_problem_file = scriptOutput + "/action_costs_from_problem_file.txt"
    ...
    if(numMaxOfWhenPerAction == 0):	# No hay efectos condicionales
        ...
        name = adl_2_strips + " -o " + domain_wac + " -f " + problem_wac + " -s " + scriptOutput  + "/ -B"
        data = os.system(name)
        ...
        name = adl_2_strips + " -o " + domain_wac + " -f " + problem_wac + " -s " + scriptOutput + "/"
        data = os.system(name)
