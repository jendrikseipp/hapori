...
void RedBlackDAGFactFollowingHeuristic::initialize() {
    ...
    if (num_invertible_vars == 0) {
    	...
    	ofstream outfile;
        cout << "?,?,?,?,?,?,?,?"<<endl;
        outfile.open("red-black", ios::out);
        outfile <<  "?,?,?,?,?,?,?" <<endl;
        cout<<"initialize: No invertible  "<<endl;
        outfile.close();
    	//isa
...
//		cout << endl;
    ...
    cout << count_black<<","<< count_root<<"," <<g_variable_domain.size() <<","<< count_all_pairs_connected;
    cout <<","<<count_all_connected_to_goal <<","<<count_almost_root<<","<<max_side_effect_for_black<<endl;
    outfile.open("red-black", ios::out);
    outfile << count_black<<","<< count_root<<"," <<g_variable_domain.size() <<","<< count_all_pairs_connected;
    outfile <<","<<count_all_connected_to_goal <<","<<count_almost_root<<","<<max_side_effect_for_black<<endl;
    cout<<"heree"<<endl;
    outfile.close();
    //Isa
}
