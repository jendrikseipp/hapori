...
if __name__ == '__main__':
    ...
    default_timeouts = [15, 30, 45, 45, 45, 45, 45, 45]
    ...
    try:
        if(original_data):
            ...
            command = "python2.7 "+ actual_rootpath + "/joinFile.py " + rootpathOutput[:rootpath.rfind("/")]
            print "Run command: " + str(command)
            os.system(command)
    ...
    if((accumulated_time < timelimit) and ((counter == 1) or ((counter > 1) and (not optimal_planning)))):
        ...
        if((accumulated_time < timelimit) and ((counter == 1) or ((counter > 1) and (not optimal_planning)))):
            ...
            time_end = int((timelimit - accumulated_time)/3)
