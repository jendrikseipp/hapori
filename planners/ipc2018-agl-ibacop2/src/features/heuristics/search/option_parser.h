...
template <>
class TokenParser<DoubleScalarEvaluator *> {
public:
    static inline DoubleScalarEvaluator *parse(OptionParser &p);
};
...
DoubleScalarEvaluator *TokenParser<DoubleScalarEvaluator *>::parse(OptionParser &p) {
    ParseTree::iterator pt = p.get_parse_tree()->begin();
    if (Registry<DoubleScalarEvaluator *>::instance()->contains(pt->value)) {
        return Registry<DoubleScalarEvaluator *>::instance()->get(pt->value) (p);
    }
    p.error("double scalar evaluator " + pt->value + " not found");
    return 0;
}
