...
void write_ltl_static_facts(FILE *ex_file){
  ...
  Facts *flist;
   ...
   for(i = 1; i < gnum_orig_predicates; i++){
	 ...
	 write_fact_prolog(ex_file, fact, PRED_LTL_STATIC);
       }
     }
   }
   for (flist = gStatic_literals; flist !=NULL; flist=flist->next){
     write_fact_prolog(ex_file, flist->fact, PRED_LTL_STATIC);
   }
}
...
//     return;
  ...
  fprintf(ex_file, "\n%% Action: "); 
  fprint_op_name(ex_file, node->op);
  ...
  for(i=0; i < parent->S.num_F; i++){
    write_fact_prolog(ex_file, &(grelevant_facts[parent->S.F[i]]), PRED_LTL_STATE);
  }
...
and if the acion to achieved was the same */
...
//     check if the node is itself then it is not repeated
//     check if the state comes from different op then the example is relevant
//     if (h->bfs_node == node || h->bfs_node->op != node->op)
    if (h->bfs_node == node){
      continue;
    }
...
void tag_commutative_action(int op, int op_next, BfsNode *node){
  BfsNode *child, *cm_next;
  
  if (node){
    for (child=node->children; child!=NULL; child=child->child_next){
      if (child->op==op && child->tag == 0){
	child->tag = 1;
	cm_next = child;
	break;
      }}
    for (child=cm_next->children; child!=NULL; child=child->child_next){
      if (child->op==op_next && child->tag == 0){
	child->tag = 3;
	break;
      }}
...
// 	  printf("tag repeated!");
      ...
      /*if (gcmd_line.full_commutative_pruning) {
	next = iSolPath->next;
	if (next){
	  if (next->node->cm_repeated)
	    tag_commutative_action(next->node->op, iSolPath->node->op, iSolPath->node->father);
	}
      }*/
    }
  }
...
{ 
  ...
  gSearch_tree->tag = 1; // The rest of the tree is already tagged!*/
...
// Comentado para generar la version para Isabel  
//   sprintf(filepath, "%s%s%s", gcmd_line.path, DP_TREES_DIR, "planning-episodes.kb");
  sprintf(filepath, "%s%s", gcmd_line.knowledge_path, "planning-episodes.kb");
