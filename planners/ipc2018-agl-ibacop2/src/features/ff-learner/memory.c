...

{
  ...
  result->is_lookahead = FALSE;
  result->lookahead_generated = FALSE;

  result->children = NULL;
  result->child_next = NULL;
  result->repeated_list = NULL;
  result->tag = 0;
  result->cm_repeated = FALSE;
  result->number = 0;
  result->repeated = FALSE;
  result->bad_repeated = FALSE;
...
OpenApp *new_OpenApp( void )

{

  OpenApp *result = ( OpenApp * ) calloc( 1, sizeof(OpenApp) );
  CHECK_PTR(result);
...
void free_OpenAppList( OpenApp *list)
{
  OpenApp *i, *aux;
