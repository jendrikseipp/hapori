/*
 * THIS SOUCE CODE IS SUPPLIED  ``AS IS'' WITHOUT WARRANTY OF ANY KIND, 
...
/*   get_landmarks_naive(&S);
     printf("\n----");
     get_landmarks(&S,FALSE);
*/
...

{
  ...
  while ( TRUE ) {
    ...
    /* Admissible vs Inadmissible pruning */
    if (!gcmd_line.adm_pruning){
      if ((first->g + first->h) > Search_upper_bound || first->bad_repeated){
	first->closed = TRUE;
	first->next = lbfs_space_had;
	lbfs_space_had = first;
	continue;
      ...
      if ((first->g + first->adm_h > Search_upper_bound) || first->bad_repeated){
	first->closed = TRUE;
	first->next = lbfs_space_had;
	lbfs_space_had = first;
	continue;
	...
	ggenerated_atfirst = ggenerated_nodes;
	printf("\n New cost bound [g:%d] with %d generated nodes",first->g, ggenerated_nodes);
	 ...
	 to zero*/
      ...
      if (gcmd_line.display_info == 200){
	extract_plan( first );
	print_plan();  
      }


      add_found_solution( first , FALSE);
...
// 	break;
    ...
    //Stop due to node bound
    if (Sols_for_learning != NULL && gcmd_line.node_bound > 0){
      if (ggenerated_nodes > gcmd_line.node_bound){
      printf ("\n Stopping search on node bound... ");
      printf ("\n Node bound %d vs generated %d", gcmd_line.node_bound, ggenerated_nodes);
      return TRUE;
      }
    }
...
{
  BfsNode *new, *i, *rep_node, *last_child, *aux;
  int j;
  Bool already_evaluated = FALSE;
  Bool better_path = FALSE;
  Bool hash_newnode = TRUE;
  Bool include_in_open = FALSE;
  BfsNode new_aux;
  BfsNodeList *nodelist;
  
  new_aux.bad_repeated = FALSE;
  ...
  if ((rep_node = same_no_metric_bfs_state_hashed( S,father ))){
    ...
    */
    ...
    /* remove hashed node from open  */
    if (rep_node->h != INFINITY){ /* we alse hash the dead end for not evaluating it again */
      if (rep_node->g > new_aux.g){
	better_path = TRUE;
	include_in_open = TRUE;
	...
	rep_node->bad_repeated = TRUE;
	unhash_bfs_node(rep_node);
      }
      else  /* g value of the old node is less or equal, thus do not hash the same node*/
	{
	  if (gcmd_line.check_repeated) /* return if we are not considering repeated nodes*/
	    return;
	  
	  hash_newnode = FALSE;
	  /* If g values are equal, keep them for different solutions */
	  /* If new g is higher mark as bad repeated*/
	  if (rep_node->g == new_aux.g)
	    include_in_open = TRUE;
	  
	  else /* (rep_node->g < new_aux.g)*/
	    new_aux.bad_repeated = TRUE;
...
/* create the new node and include it in the found position in open */
  ...
  new->father = father;
  new->g = new_aux.g;
  new->bad_repeated = new_aux.bad_repeated;
  new->number = ggenerated_nodes++;

//   new->cm_repeated = new_aux.cm_repeated;
  ...
  /*do not include in open nodes are not needed. However they are created and linked to their father */
  if (!include_in_open){
    new->closed = TRUE; 
	...
	new->gnum_RP = gnum_RP;
      ...
      {
      copy_BfsNode_evaluated(&new_aux, new);
      if (!better_path)
	new->repeated = TRUE;
      }
    ...
    if (!already_evaluated || better_path || !gcmd_line.save_repeated_lists){
      for ( i = lbfs_space_head; i->next; i = i->next ) {
	if ( i->next->int_fn >= new_aux.int_fn )
	  break;
      }
	...
	new->next->prev = new;
      ...
      nodelist = ( BfsNodeList * ) calloc( 1, sizeof( BfsNodeList ) );
      nodelist->node = new;
      
      aux = rep_node->repeated_list;
      rep_node->repeated_list = nodelist;
      nodelist->next = aux;
      
    }
  } /*endif not included in open*/
...
void full_clean_commutative_states(void){
  BfsNode *i,*rep_node;
//   print_list(lbfs_secondary_current_head);
  
  for ( i = lbfs_secondary_current_head; i->next; i = i->next ) {
    if (i->father){
      if (commutative_actions(i->op, i->father->op) && !(i->h==0)){
//       if (i->cm_repeated){
// 	printf ("\n commutative state cleaned up...");
// 	print_op_name (i->op); 
// 	printf ("(%d) \n", i->g);
	if (i != lbfs_secondary_current_head)
	  i->prev->next = i->next;
	else{
	  lbfs_secondary_current_head = i->next;
	}
...
void clean_sols_commutative_states(void){
  BfsNode *i,*rep_node;
...
//   print_list(lbfs_secondary_current_head);
  ...
  for ( i = lbfs_secondary_current_head; i->next; i = i->next ) {
    ...
    if ((rep_node = same_no_metric_bfs_state_hashed(&(i->S),NULL ))){
      if (rep_node->cm_repeated){
	printf ("\n commutative state cleaned up...");
	print_op_name (i->op); 
	printf ("(%d) \n", i->g);
	if (i != lbfs_secondary_current_head)
...
/*This mark the node as good solutions and all other 
  nodes up to a existing good branch.*/
void mark_repeated_solution_path (BfsNode *node){
  BfsNode *i;
  int updates = 0;

  for (i = node; i != NULL; i = i->father){
    if (i->tag > 0)
      break;
    
    i->tag = 3; // in solution, but repeated
//     if (updates > 0)
//       printf("\n upward node marked!");
    updates++;
  }
...
Bool clean_repeated_already_in_solution(void){
  BfsNode *i,*rep_node;
  Bool something_pruned = FALSE;
  long nodecount =0;
  printf("\n ____ ENTERING REPEATED CLEANER ____");
  //print_list(lbfs_secondary_current_head);
  
  for ( i = lbfs_secondary_current_head; i != NULL; i = i->next ) 
    nodecount++;
  printf (" %ld nodes in secondary" , nodecount);
...
//       printf(" already updated");
    ...
    if (i->cm_repeated){
	mark_repeated_solution_path(i);
 	printf("\n already cm_marked");
	continue;
    }
    
    if ((rep_node = same_no_metric_bfs_state_hashed(&(i->S),NULL ))){
      if (rep_node->tag > 0 && i->g == rep_node->g){
	mark_repeated_solution_path(i);
 	printf("\n repeated found in solution");
	continue;
      }
    }
//     printf(" anything done!");
  }
    
//     removing from secondary list 
  for ( i = lbfs_secondary_current_head; i->next; i = i->next ) {
    if (i->tag > 0){
      something_pruned = TRUE;
      printf("\n removing node %ld", i->number);
      if (i != lbfs_secondary_current_head)
	i->prev->next = i->next;
      else{
	lbfs_secondary_current_head = i->next;
...
Bool do_anytime_bfs_bnb_learn_delay_repeat(void)
{
  ...
  add_to_anytime_bfs_bnb_learn_delay_repeat_space( &ginitial_state, -1, NULL );
  ...
  while ( TRUE ) {
    ...
    /* Admissible vs Inadmissible pruning */
    if (!gcmd_line.adm_pruning){
      if (first->int_fn > Search_upper_bound || first->bad_repeated){
	first->closed = TRUE;
	first->next = lbfs_space_had;
	lbfs_space_had = first;
	continue;
...
// //       output_planner_info();
      ...
      /* next break is to stop at first solution: commented */
//       Note that I use this as the opposite
      if (!gcmd_line.stop_at_first_sol)
	break;
    ...
    /*if (first->op != -1){
      printf("\n >> Expanding node:");
      printf ("\n >> Node: g:%d ,h:%d , f:%d >> ", first->g, first->h, first->int_fn);
      print_op_name(first->op);
      }*/
	 ...
	 * is entirely fulfilled; only those actions are applied.
	 */
	add_to_anytime_bfs_bnb_learn_delay_repeat_space( &S, first->H[i], first );
      }
    }
	 ...
	 * is entirely fulfilled; only those actions are applied.
	 */
	add_to_anytime_bfs_bnb_learn_delay_repeat_space( &S, first->nonH[i], first );
      }
    }
...
void add_to_anytime_bfs_bnb_learn_delay_repeat_space( State *S, int op, BfsNode *father )
{
  BfsNode *new, *i, *rep_node, *last_child, *aux;
  int j;
  Bool already_evaluated = FALSE;
  ...
  if ((rep_node = same_no_metric_bfs_state_hashed( S,father ))){
    ...
    /*if ( gcmd_line.solution_path_pruning ) {
      if (rep_node->tag> 0 && rep_node->g == new_aux.g){
 	//printf("\n Marking state in solution path...");
	new_aux.cm_repeated = TRUE;
      }
      }*/
    
    /* copy evaluation data in the variables of the new future node to avoid a re-evaluation */
    copy_BfsNode_evaluated(rep_node,&new_aux);
    ...
    else  /* g values are less or equal, thus do not hash the same node*/
      {
	hash_newnode = FALSE;
	/* If g values are equal, keep them for different solutions */
	/* If new g is higher mark as bad repeated*/
	if (rep_node->g == new_aux.g){
	  if (!gcmd_line.check_repeated)
	    include_in_open = TRUE;
	}
	else /* (rep_node->g < new_aux.g)*/
	  {
...
/* create the new node and include it in the found position in open */
  ...
  /*Do not insert the new node in open if is repeated from a previous solution*/
  /*  if (new->cm_repeated){
    mark_repeated_solution_path(new);
    new->closed = TRUE;
    return;
    }*/
  ...
  if (!already_evaluated || better_path){
    /*printf ("\n Inserted in open ");*/
    for ( i = lbfs_space_head; i->next; i = i->next ) {
      if ( i->next->int_fn >= new_aux.int_fn )
	break;
  ...
  if (!already_evaluated || better_path /*|| lbfs_secondary_current_head*/){
    /*printf("..ok \n");*/
    new->next = i->next;
    new->prev = i;
    i->next = new;
  ...
  while ( TRUE ) {
    ...
    /* generate lookahead states */
     ...
     if (!first->is_lookahead )
       lookahead = obtain_lookahead_state_order(first);
     else
      lookahead = NULL;
	...
	if (added_node){
	  ...
	  */
	  ...
	  lookahead = obtain_lookahead_state_order(added_node);
...

{
  ...
  BfsNode *first, *added, *aux, *node_most_near = NULL;
  State S;
  int i, min = INFINITY;
  Bool sstart = TRUE;
  struct tms start, end;
  Bool goals_achieved = FALSE;
  ...
  /* to compute which operators delete top level goals: */
  deactivate_actions_deleting_top_level_logic_goals();
  activate_all();
  ...
  /* extract first from open */
    ...
    */
    ...
    //     first->h = get_1P_and_H( &(first->S));

    if (gcmd_line.steps_empty_goal == 0 && !goals_achieved)
      {
	first->h = get_1P( &(first->S));
     //     printf("\n h = %d", first->h);
	if (first->h == INFINITY)
	  continue;
      }
    ...
    if (!goals_achieved && gcmd_line.steps_with_goals > 0 && (gcmd_line.steps_empty_goal == 0 && first->h == 0))
      {
	goals_achieved = TRUE;
	printf("\Goals achieved, tried %d steps more ahead (total %d)", 
	       gcmd_line.steps_with_goals - first->g, gcmd_line.steps_with_goals );
      }

    /*
    if (goals_achieved && first->h == 0)
      {
	if (!node_most_near || 
	    ((gcmd_line.steps_with_goals - first->g) < (gcmd_line.steps_with_goals - node_most_near->g)))
	  node_most_near = first;
      }
    ...
    */
	...
	if (goals_achieved)
	  {
	    if (!gop_conn[gH[i]].deletes_top_level_goal)
	      {
		added = add_to_depth_first_space_roller( &S, gH[i], first );
		if (added != NULL)
		  break;
	      }
	  }
    ...
    if (  (gcmd_line.steps_empty_goal == 0 &&  gcmd_line.steps_with_goals == 0 && first->h == 0) || 
	  
	  (gcmd_line.steps_empty_goal > 0 && (gcmd_line.steps_empty_goal == first->g ||  lbfs_space_head->next == NULL)) ||

	  (goals_achieved &&  (gcmd_line.steps_with_goals <= first->g  || !added)))
...
/* This checks for target goals that can be achieved be applicable actions that have no delete effects.
   This greedy application is harmless. You need to call first recompute_target_goals*/
Bool set_candidate_harmless(){
  int goal, i,j, achiever;
  Bool something_harmless = FALSE;

  for(i = 0; i < gnum_flogic_goal; i++)
    {
      goal = gflogic_goal[i]; /* fact index */
      for (j = 0; j < gft_conn[goal].num_A; j++)
	{
	  achiever = gft_conn[goal].A[j];
	  if (gef_conn[achiever].num_D == 0){
	    gop_conn[gef_conn[achiever].op].harmless = TRUE;
	    something_harmless = TRUE;
	  }
	}
     
    }
  return something_harmless;
}



/* ROLLER search*/
Bool do_depth_first_chronological_backtraking_roller()

{
  ...
  Bool sstart = TRUE;
  Bool next_harmless = FALSE;
  Bool apply_harmless = FALSE;
  struct tms start, end;
  ...
  /* setting all actions as not harmless anyway*/
  for(i = 0; i < gnum_op_conn; i++)
    gop_conn[i].harmless = FALSE;
  
  if (gcmd_line.apply_harmless == TRUE && set_candidate_harmless())
    apply_harmless = TRUE;
  ...
  while ( TRUE ) {
    ...
    assign_LT_heuristic_info(first);
    
    /* checks for harmless application. If TRUE wi skip roller matching */
    if (apply_harmless){
      next_harmless = FALSE;
      for ( i = gnum_H - 1; i >= 0; i-- ) {                                                                                             
	if (gop_conn[gH[i]].harmless)
	  next_harmless = TRUE;
      }
    }
    ...
    /* add to the front of the open list the harmless actions */
    if (next_harmless){
      for ( i = gnum_H - 1; i >= 0; i-- ) {                                                                                             
	if (gop_conn[gH[i]].harmless){
	  if ( result_to_dest( &S, &(first->S), gH[i]) ){
...
// This is a copy of the modified_hill_climbing, but for stopping at a given horizon
// for getting the final state for problem generation. 
// We use the same add_to_space function
Bool do_hill_climbing_horizon( void )

{
  ...
  while ( TRUE ) {
    ...
    /* compute the solution cost: final_total_cost  */
    if ( first->h == 0 || 
	 (gcmd_line.steps_empty_goal > 0 && gcmd_line.steps_empty_goal == first->g)) {
      if ( gcmd_line.optimize && goptimization_established ){
	final_total_cost=first->float_gn;
      }
      else{
	...
	output_planner_info_to_file();
      ...
      if ( first->h == 0){
	printf ("\n Goals reached within horizon!\n");
      }
      else {
	printf ("\n Stopping at horizon, Goals not reached!\n");
      }
      printf("\n\nDIP-Generator INITIAL STATE:\n");
      print_static_facts();
      print_State( (first->S));
...
// A computation of the initial state features in order to predict the search time

Bool compute_init_state_features(void)

{
  ...
  printf("\n\n INITIAL STATE FEEATURES\n");
  printf("\n num_relevant_facts: %d ", gnum_relevant_facts);
  printf("\n num_actions: %d ", gnum_actions);
  make_state( &S, gnum_ft_conn, gnum_fl_conn );
  ...
  feature_hmax = first->adm_h;
  feature_hff = first->h;
  feature_hff_ratio = first->h * 1.0 / first->adm_h;

  printf("\n h_ff: %d", first->h);
  printf("\n h_max: %d", first->adm_h);
  printf("\n h_ffratio %8.2f", feature_hff_ratio);
//   printf("\n h_max_momentum %d",feature_hmax_momentum);
//   printf("\Fixpoint:");
  ...
  for (i = gnum_RP-1; i>=0; i--)
    {
      ...
      printf("\n");
      print_op_name( op );
      printf(" :%d ", gop_conn[op].is_used);
    }
  */
  
  
  /*   solvable = build_fixpoint( &S, &max_levels);
  printf("\n\n Relaxed Graphplan\n");
  for ( i = 0; i < gnum_op_conn; i++ ) {      
    printf("\n");
    print_op_name( i );
    printf(": %d ", gop_conn[i].level_achievable);
  }
  */
...
/* ROLLER search*/
Bool do_dptrees_depth_first(Bool with_nonHA)
{
  ...
  add_to_depth_first_space_dptrees( &ginitial_state, -1, NULL );

  /* load roller trees */
  load_tilde_dp_clauses();
  load_tilde_dp_trees();
  ...
  while ( TRUE ) {
    ...
    if ( (first = lbfs_space_head->next) == NULL ) {
      ...
      */
      ...
      /* include all nodes in the secondary list*/                                                                                    
      include_list_in_open_elements(lbfs_secondary_current_head,lbfs_secondary_current_tail, add_to_depth_first_space_dptrees,1);         
...
//     assign_LT_heuristic_info(first);
    ...
    for ( i = gnum_H - 1; i >= 0; i-- ) {
      if ( result_to_dest( &S, &(first->S), gH[i] ) ) {
	if (gop_conn[gH[i]].roller_value > 0)
	  add_to_depth_first_space_dptrees( &S, gH[i], first );
      }
    }
    */
...
BfsNode * add_to_depth_first_space_dptrees( State *S, int op, BfsNode *father) 
{
...
/* With secondary list for non-helpful sucessors */
Bool do_anytime_bfs_roller_policy_SEC( void )

{
  ...
  while ( TRUE ) {
    ...
    if ( (first = lbfs_space_head->next) == NULL ) {
      /* include one node of the secondary list*/
      include_list_in_open_elements(lbfs_secondary_current_head,lbfs_secondary_current_tail,
				    add_to_anytime_bfs_roller_policy,1);
      lbfs_secondary_current_head = (BfsNode*)0;
      lbfs_secondary_current_tail = (BfsNode*)0;
...
Bool do_anytime_bfs_FF_heuristic_policy_SEC( void )
...

{
  ...
  while ( TRUE ) {
	 ...
	 */
	//add_to_secondary_list( &S, first->H[i], first );
	add_to_anytime_bfs_HA_rescue_space_roller_policy( &S,  first->H[i], first );
      }
    }
	 ...
	 */
	//add_to_secondary_list( &S, first->nonH[i], first );
	add_to_anytime_bfs_HA_rescue_space_roller_policy( &S,  first->nonH[i], first );
      }
    }
