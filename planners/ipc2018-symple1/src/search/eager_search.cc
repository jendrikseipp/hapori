...
#include "plugin.h"
#include "sym_evmdd/operationTransitionCreater.h"
...
void EagerSearch::initialize() {
    ...
    MEDDLY::domain* dom = MEDDLY::createDomain(vars, nVars);
    MEDDLY::forest* forest = dom->createForest(false, MEDDLY::forest::INTEGER,
...
int EagerSearch::step() {
    ...
    for (int i = 0; i < applicable_ops.size(); i++) {
        ...
        if ((node.get_real_g() + op->get_cost(s)) >= bound)
            continue;
        ...
        if (succ_node.is_new()) {
            ...
            open_list->evaluate(node.get_g() + get_adjusted_cost(s, *op), is_preferred);
            ...
            if (do_pathmax) {
                if ((node.get_h() - get_adjusted_cost(s, *op)) > succ_h) {
                    //cout << "Pathmax correction: " << succ_h << " -> " << node.get_h() - get_adjusted_cost(*op) << endl;
                    succ_h = node.get_h() - get_adjusted_cost(s, *op);
        ...
        } else if (succ_node.get_g() > node.get_g() + get_adjusted_cost(s, *op)) {
