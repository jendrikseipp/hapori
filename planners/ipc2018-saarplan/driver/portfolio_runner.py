...
def adapt_args(args, search_cost_type, heuristic_cost_type, plan_manager, reduce_bound_by_one=False):
    g_bound = plan_manager.get_best_plan_cost()
    if (reduce_bound_by_one and g_bound != "infinity"):
        g_bound = max(0, int(g_bound) - 1)
    ...
    for index, arg in enumerate(args):
        if arg == "--heuristic":
            ...
            if "bound=BOUND" not in search:
                ...
                search = search.replace("bound=BOUND", "bound=" + str(g_bound)) # need this here separately to not overwrite other parameters containing "BOUND"
            for name, value in [
...
def run_sat(configs, executable, sas_file, plan_manager, final_config,
    ...
    if (original_executables):
        executables = original_executables
    while configs:
        configs_next_round = []
        executables_next_round = []
        for pos, (relative_time, args) in enumerate(configs):
            if (executables is not None):
                executable = executables[pos]
            ...
            if exitcode == EXIT_PLAN_FOUND:
                ...
                if (executables is not None):
                    executables_next_round.append(executable)
        ...
        executables = executables_next_round
...
def run_opt(configs, executable, sas_file, plan_manager, remaining_time_at_start,
            memory, planner_args, executables):
    exitcodes = []
    for pos, (relative_time, args) in enumerate(configs):
        timeout = determine_timeout(remaining_time_at_start, configs, pos)
        if (executables is not None):
            executable = executables[pos]
        if (planner_args.cost_bound is not None):# a bit hacky, for bounded-cost track
            for index, arg in enumerate(args):
                if arg == "--search":
                    search = args[index + 1]
                    if "bound=BOUND" in search:
                        bound = int(planner_args.cost_bound)
                        if (executable.endswith("fast-downward-decoupled/search/downward-release")): # HACK: cost bound is handled differently in the two planners
                            bound = bound + 1 # HACK for decoupled search
                        search = search.replace("bound=BOUND", "bound=" + str(bound))
                        print("set cost bound to " + str(bound))
                        args[index + 1] = search
                    break
...
def run(portfolio, executable, sas_file, plan_manager, args):
    ...
    if (executables is not None):
        ...
        for i in range(len(executables)):
            executables[i] = os.path.join(curr_path, executables[i])
    ...
    print("remaining time at start: %.2f" % (remaining_time_at_start - get_elapsed_time()))

    if optimal:
        exitcodes = run_opt(configs, executable, sas_file, plan_manager,
                            remaining_time_at_start, memory, args, executables)
    else:
        exitcodes = run_sat(configs, executable, sas_file, plan_manager,
                            final_config, final_config_builder,
                            remaining_time_at_start, memory, executables)
