...
int pddlGAddNode(pddl_g_t *g)
{
    ...
    if (g->node_size >= g->node_alloc){
        if (g->node_alloc == 0)
            g->node_alloc = 1;
        g->node_alloc *= 2;
        g->node = BOR_REALLOC_ARR(g->node, pddl_g_node_t, g->node_alloc);
    }
...
int pddlGAddEdge(pddl_g_t *g, int from_node, int to_node)
{
    ...
    if (g->edge_size >= g->edge_alloc){
        if (g->edge_alloc == 0)
            g->edge_alloc = 1;
        g->edge_alloc *= 2;
        g->edge = BOR_REALLOC_ARR(g->edge, pddl_g_edge_t, g->edge_alloc);
    }
...
static int pddlGInduceNode(const pddl_g_t *g, int node, pddl_g_t *dst)
{
    int dnode = pddlGAddNode(dst);
                 ...
                 const bor_iset_t *edges)
{
    ...
    if (nodes != NULL){
        BOR_ISET_FOR_EACH(nodes, node)
            node_map[node] = pddlGInduceNode(g, node, dst);
    }else{
        BOR_ISET_FOR_EACH(edges, edge){
            const pddl_g_edge_t *e = g->edge + edge;
            if (node_map[e->from] == -1)
                node_map[e->from] = pddlGInduceNode(g, e->from, dst);
            if (node_map[e->to] == -1)
                node_map[e->to] = pddlGInduceNode(g, e->to, dst);
        }
    }
    ...
    for (int edge = 0; edge < g->edge_size; ++edge){
        ...
        if (node_map[e->from] >= 0 && node_map[e->to] >= 0){
            dedge = pddlGAddEdge(dst, node_map[e->from], node_map[e->to]);
...
static void sccTarjanExtract(pddl_g_t *scc, int *scc_node_map,
                             scc_dfs_t *dfs, int nid)
{
    ...
    dst_node = pddlGAddNode(scc);

    // Copy node IDs from stack to the component
    for (int j = 0; j < depth; ++j){
        pddlGNodeAddLabel(scc, dst_node, dfs->stack[i + j]);
...
static void sccTarjanStrongconnect(pddl_g_t *scc, int *scc_node_map,
                                   const pddl_g_t *g,
                                   scc_dfs_t *dfs, int nid)
{
    int w, edge_id;
    const pddl_g_node_t *node = g->node + nid;
...
static void sccAddEdges(pddl_g_t *scc, const int *scc_node_map,
                        const pddl_g_t *g)
{
    for (int i = 0; i < g->edge_size; ++i){
        const pddl_g_edge_t *edge = g->edge + i;
        ASSERT(scc_node_map[edge->from] >= 0);
        ASSERT(scc_node_map[edge->to] >= 0);
        int f = scc_node_map[edge->from];
        int t = scc_node_map[edge->to];
        if (f != t){
            int e = pddlGGetOrAddEdge(scc, f, t);
            pddl_g_edge_t *dst = scc->edge + e;
...
static void dfs(const pddl_g_t *g, int from, const bor_iset_t *ignore,
                int *visited)
{
    int edge_id;
                   ...
                   void *ud)
{
    ...
    for (int i = 0; i < g->edge_size; ++i){
        const pddl_g_edge_t *e = g->edge + i;
