...
static void heurFlow(plan_heur_t *_heur, const plan_state_t *state,
                     plan_heur_res_t *res);
static void heurFlowMGroupLdm(plan_heur_t *_heur,
                                      ...
                                      size_t max_mem)
{
    ...
    hflow = BOR_ALLOC(plan_heur_flow_t);
    bzero(hflow, sizeof(*hflow));
    _planHeurInit(&hflow->heur, heurFlowDel, NULL, heurFlowMGroupLdm);
    hflow->use_ilp = (flags & PLAN_HEUR_FLOW_ILP);
    ...
    pddlHeurMGroupMergeInit(&merge, pddl->strips, 0, max_mem);
    if (merge.merge_size > 0){
        hflow->ldm_seq_size = merge.merge_size;
        hflow->ldm_seq = BOR_CALLOC_ARR(pddl_landmark_seq_t,
                                        hflow->ldm_seq_size);
        for (int i = 0; i < merge.merge_size; ++i)
            pddlLandmarkSeqCopy(hflow->ldm_seq + i, &merge.merge[i].ldm_seq);
    }
    pddlHeurMGroupMergeFree(&merge);
    hflow->ldm_seq_pool = -1;
...
static void heurFlowDel(plan_heur_t *_heur)
{
    ...
    for (int i = 0; i < hflow->ldm_seq_size; ++i)
        pddlLandmarkSeqFree(hflow->ldm_seq + i);
    if (hflow->ldm_seq != NULL)
        BOR_FREE(hflow->ldm_seq);
...
static void heurFlowMGroupLdm(plan_heur_t *_heur,
                              ...
                              plan_heur_res_t *res)
{
    plan_heur_flow_t *h = HEUR(_heur);

    if (h->ldm_seq_pool < 0){
        int el[h->ldm_seq_size];
        for (int i = 0; i < h->ldm_seq_size; ++i)
            el[i] = -1;
        h->ldm_seq_pool = planStatePoolDataReserve(search->state_pool,
                                sizeof(int) * h->ldm_seq_size,
                                NULL, el);
    }

    int *ldm_track = planStatePoolData(search->state_pool, h->ldm_seq_pool,
                                       state_id);
    const plan_state_t *state = planSearchLoadState(search, state_id);
    const plan_state_space_node_t *node = planSearchLoadNode(search, state_id);

    if (node->op == NULL){
        for (int i = 0; i < h->ldm_seq_size; ++i)
            ldm_track[i] = 0;
    }else{
        const int *parent = planStatePoolData(search->state_pool,
                                              h->ldm_seq_pool,
                                              node->parent_state_id);
        int op_id = node->op->global_id;
        for (int i = 0; i < h->ldm_seq_size; ++i){
            ldm_track[i] = parent[i];

            const pddl_landmark_seq_t *lseq = h->ldm_seq + i;
            for (int j = ldm_track[i]; j < lseq->ldm_size; ++j){
                if (borISetIn(op_id, lseq->ldm + j)){
                    ++ldm_track[i];
                }else{
                    break;
                }
    ...
    for (int i = 0; i < h->ldm_seq_size; ++i){
        const pddl_landmark_seq_t *lseq = h->ldm_seq + i;
        for (int j = ldm_track[i]; j < lseq->ldm_size; ++j){
            planLandmarkSetAdd(&ldms, lseq->ldm[j].size, lseq->ldm[j].s);
        }
    }
    ...
    if (h->lm_cut){
        ...
        planHeurResInit(&ldms_res);
        ldms_res.save_landmarks = 1;
        planHeurState(h->lm_cut, state, &ldms_res);
        for (int i = 0; i < ldms_res.landmarks.size; ++i){
            planLandmarkSetAdd(&ldms, ldms_res.landmarks.landmark[i].size,
                                      ldms_res.landmarks.landmark[i].op_id);
        }
        planLandmarkSetFree(&ldms_res.landmarks);
    }

    factsSetState(h->facts, &h->fact_id, state);
    res->heur = lpSolve(h->lp, h->facts, h->fact_id.fact_size,
                        h->use_ilp, &ldms);
    planLandmarkSetFree(&ldms);
}
