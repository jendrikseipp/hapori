...
int main(int argc, const char ** argv) {
  ...
  read_preprocessed_problem_description(cin, metric, internal_variables,
  ...
  // compute h2 mutexes
  if (axioms.size() > 0)
    std::cout << "Axioms: h2 not used." << std::endl;
  if (h2_mutex_time && axioms.size() == 0) {
    ...
    compute_h2_mutexes(ordering, operators, axioms, mutexes, initial_state,
    goals, h2_mutex_time, conditional_effects);
  ...
  generate_cpp_input(solveable_in_poly_time, ordering, metric, mutexes,
  initial_state, goals, operators, axioms, successor_generator,
