#include "iterated_search_aras_rw.h"
#include <limits>
#include <fstream>
#include <time.h>

IteratedSearchArasRW::IteratedSearchArasRW(const SearchEngineOptions &options,
        const vector<string> &engine_config_,
        vector<int> engine_config_start_, bool pass_bound_,
        bool repeat_last_phase_, bool continue_on_fail_,
        bool continue_on_solve_, int plan_counter_) :
    IteratedSearchAras(options, engine_config_, engine_config_start_,
            pass_bound_, repeat_last_phase_, continue_on_fail_,
            continue_on_solve_, plan_counter_)
{
    m_stopTimes.clear();
    int time_interval = 1800 / NUM_RESTART;
    for (int i = 0; i < NUM_RESTART; ++i)
        m_stopTimes.push_back(m_startTime + (i+1) * time_interval);

}

IteratedSearchArasRW::~IteratedSearchArasRW() {
}


int IteratedSearchArasRW::step() {

    for (int i = 0; i < NUM_RESTART; ++i) {
        g_search_stopTime = m_stopTimes[i];
        phase = 0;
        best_bound = numeric_limits<int>::max();

//        if (i == 0)
//            g_random_bucket_remove_min = false;
//        else
//            g_random_bucket_remove_min = false;
        g_rand_open = (i == 0)? false : true;
        while (iterated_aras_step() == IN_PROGRESS)
            ;
    }

    return SOLVED;
}


int IteratedSearchArasRW::iterated_aras_step() {
    current_search = create_phase(phase);
    if (current_search == NULL) {
        return found_solution() ? SOLVED : FAILED;
    }
    if (pass_bound) {
        current_search->set_bound(best_bound);
    }
    phase++;

    if (iterated_found_solution)
        g_turn_on_search_early_stop = true;
    current_search->search();
    g_turn_on_search_early_stop = false;
    if (g_quit_for_timeout == true)
    {
        g_quit_for_timeout = false;
        cout << "time out! " << m_startTime << ":" << get_time(g_time_deduct) << endl;
        return FAILED;
    }

    SearchEngine::Plan found_plan;
    int plan_cost = 0;
    last_phase_found_solution = current_search->found_solution();
    if (last_phase_found_solution) {
        iterated_found_solution = true;
        found_plan = current_search->get_plan();
        plan_cost = calculate_plan_cost(found_plan);
        if (plan_cost < best_bound) {
            best_bound = plan_cost;
            set_plan(found_plan);
        }

        //check if first plan exceed stoptime
        //if that, not using restart
        bool first_soln_exceed_stoptime = false;
        if (m_best_cost == numeric_limits<int>::max())
            if (get_time(g_time_deduct) > g_search_stopTime)
                first_soln_exceed_stoptime = true;

        if (plan_cost < m_best_cost)
        {
            cout << "Real Plan cost: " << plan_cost << "." << endl;
            cout << "Real Total time: " << get_time(g_time_deduct) - m_startTime << "s" << endl;
            m_best_cost = plan_cost;
            ++plan_counter;
            save_plan(found_plan, plan_counter);
            std::cout << "plan_id: " << plan_counter << endl;
        }

        //run one plan aras
        if (USE_ARAS)
        {
            int current_best_cost = m_best_cost;
            int current_aras_start_time = get_time(g_time_deduct);
            save_plan(found_plan, -1);
            
	    string input_plan_file_name = g_plan_filename + "--1";
            string output_bound_file_name = g_plan_filename + "-output_bound";
            string aras_plan_file_name = g_plan_filename + "-aras";

            int remainning = (int) (g_search_stopTime - (get_time(g_time_deduct)) - 30);
            remainning = min(remainning, get_time_limit());
            if (first_soln_exceed_stoptime)
            {
                remainning = (1800 - (get_time(g_time_deduct) - m_startTime) - 30);
                cout << "because of first_soln_exceed_stoptime, aras will take as much time as possible" << endl;
            }
            if (remainning < 0)
            {
                cout << "time out! " << m_startTime << ":" << get_time(g_time_deduct) << endl;
                return FAILED;
            }
            cout << g_search_stopTime << "; " << get_time(g_time_deduct)%10000 << ";" << remainning << endl;
            char cmd[5000];
            sprintf(cmd, "pwd;ulimit -t %d;"
                "%s/one_run_aras -run_one_plan_aras %s %s -o %s"
                " -mrw_shared \"-iterative -run_aras -aras_time -1  -aras_mem -1\" "
                "< %s >> %s_aras_tmp_output ",  remainning,
                    get_selfpath().c_str(),
                    input_plan_file_name.c_str(),
                    output_bound_file_name.c_str(), aras_plan_file_name.c_str(),
                    g_output_file_path.c_str(), g_plan_filename.c_str());

            fprintf(stdout, "cmd: %s", cmd);

            if (remainning > 0)
            {
                std::cout << "**************start aras***************"
                        << std::endl;
                int status = system(cmd);
                if (status != 0)
                {
                    std::cout << "run system error or terminated by time or memory!" << std::endl;
                }
                std::cout << "**************end aras***************"
                        << std::endl;

                std::vector<const Operator*> aras_plan = GetPlanFromFile(aras_plan_file_name);
                if(aras_plan.size() != 0) //exist aras_plan
                {
                    int aras_plan_cost = calculate_plan_cost(aras_plan);
                    if (aras_plan_cost < m_best_cost)
                    {
                        ++plan_counter;
                        save_plan(aras_plan, plan_counter);
                    }
                }

                FILE *bound_file = fopen(output_bound_file_name.c_str(), "r");
                if (bound_file == NULL)
                {
                    std::cerr << "can not open bound_file!" << std::endl;
                }
                else
                {
                    int tmp_cost = -1;
                    int tmp_time = -1;

                    char buf[1000];
                    while (fgets(buf, 1000, bound_file) != NULL)
                    {
                        sscanf(buf, "%d %d", &tmp_cost, &tmp_time);
                        int bound = best_bound;
                        bound = tmp_cost;
                        int time = current_aras_start_time;
                        time = (current_aras_start_time + tmp_time) - m_startTime;

                        cout << "aras_plan_bound: " << bound << " " << time << endl;
                        if (bound < best_bound)
                        {
                            if (BOUND_ARAS_PLAN)
                                best_bound = bound;
                            cout << "[t=" << g_timer() << "]" << std::endl;
                            cout << "Plan-cost: " << best_bound << " ."
                                    << endl;
                            cout << "Total-time: " << time << "s" << std::endl;
                        }

                        if (bound < m_best_cost)
                        {
                            cout << "Real Plan cost: " << bound << "."
                                    << endl;
                            cout << "Real Total time: " << time << "s"
                                    << endl;
                            m_best_cost = bound;
                            std::cout << "plan_id: " << plan_counter << endl;
                        }
                    }
                }
                //bound_file.close();
                fclose(bound_file);

                if (FIRST_RUN_CHECK && m_first_run)
                {
                    if (current_best_cost == m_best_cost) //no improvment
                    {
                        USE_ARAS = false;
                        cout << "ARAS is disabled by FIRST_RUN_CHECK!!" << endl;
                    }
                }
                m_first_run = false;
            }
        }
    }
    current_search->statistics();
    search_progress.inc_expanded(
        current_search->get_search_progress().get_expanded());
    search_progress.inc_evaluated_states(
        current_search->get_search_progress().get_evaluated_states());
    search_progress.inc_evaluations(
        current_search->get_search_progress().get_evaluations());
    search_progress.inc_generated(
        current_search->get_search_progress().get_generated());
    search_progress.inc_generated_ops(
        current_search->get_search_progress().get_generated_ops());
    search_progress.inc_reopened(
        current_search->get_search_progress().get_reopened());

    return step_return_value();
}

SearchEngine *IteratedSearchArasRW::create(
    const vector<string> &config, int start, int &end, bool dry_run) {
    if (config[start + 1] != "(")
        throw ParseError(start + 1);

    SearchEngineOptions common_options;

    vector<int> engine_config_start;
    OptionParser::instance()->parse_search_engine_list(config, start + 2,
                                                       end, false, engine_config_start, true);

    if (engine_config_start.empty())
        throw ParseError(end);
    end++;

    bool pass_bound = true;
    bool repeat_last = false;
    bool continue_on_fail = false;
    bool continue_on_solve = true;
    int plan_counter = 0;

    if (config[end] != ")") {
        end++;
        NamedOptionParser option_parser;
        common_options.add_options_to_parser(option_parser);

        option_parser.add_bool_option("pass_bound", &pass_bound,
                                      "use bound from previous search");
        option_parser.add_bool_option("repeat_last", &repeat_last,
                                      "repeat last phase of search");
        option_parser.add_bool_option("continue_on_fail", &continue_on_fail,
                                      "continue search after no solution found");
        option_parser.add_bool_option("continue_on_solve", &continue_on_solve,
                                      "continue search after solution found");
        option_parser.add_int_option("plan_counter", &plan_counter, 
                                      "start enumerating plans with this number");
        option_parser.parse_options(config, end, end, dry_run);
        end++;
    }
    if (config[end] != ")")
        throw ParseError(end);

    if (dry_run)
        return NULL;

    IteratedSearchArasRW *engine = \
        new IteratedSearchArasRW(common_options,
                           config, engine_config_start, pass_bound,
                           repeat_last, continue_on_fail, continue_on_solve,
                           plan_counter);

    return engine;
}

std::vector<const Operator*> IteratedSearchArasRW::GetPlanFromFile(string file_name)
{

    std::vector<const Operator*> plan;
    ifstream input_file(file_name.c_str());
    if (!input_file.good())
    {
        std::cerr << "input plan file error! " << std::endl;
        return plan;
    }
    char op_str[200];
    while(input_file.good())
    {
        input_file.getline(op_str, 200);
        if(input_file.good())
        {
            vector<Operator>::iterator it;
            for (it = g_operators.begin(); it != g_operators.end(); ++it)
            {
                const Operator* op = &(*it);
                string op_name = op->get_name();
                op_name = "(" + op_name + ")";
                if (op_name.compare(op_str) == 0)
                {
                    plan.push_back(op);
                    break;
                }
            }

            if (it == g_operators.end())
            {
                std::cout << "operator not found in reading plan!" << endl;
                plan.clear();
                break;
            }
        }
    }
    input_file.close();

    return plan;
}


const int IteratedSearchArasRW::NUM_RESTART = 4;
