...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 7:
        ...
        if (os.path.isdir(sys.argv[1])):
            ##scriptOutput = os.path.abspath(os.path.join(currentpath,"../../..")
        ...
        for i in output_plans:
            ...
            if((not successful_plan) or (current_cost == -1)):
                print("WARNING: plan " + str(i) + " is a wrong plan file or the plan cost is equal to -1, so we remove it")
                os.system("rm -f " + i)

            elif((successful_plan) and ((best_cost != -1) and (current_cost > best_cost))):
                print("WARNING: plan " + str(i) + " is worse (" + str(current_cost) + ") than the previous plan generated (" + str(best_cost) + "), therefore we remove it")
                os.system("rm -f " + i)
