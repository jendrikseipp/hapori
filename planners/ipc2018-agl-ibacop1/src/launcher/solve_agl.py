...
if __name__ == '__main__':
    ...
    default_timeouts = [80, 30, 45, 45, 45, 45, 20]
    ...
    if (mem.total < memory):
        ...
        print "***************************\n"
        print "!!!! ----- WARNING ---!!!!"
        print "Only print this message if there is a test machine"
    original_data = True
    best_cost = -1
    counter = 1
    ...
    print "Main portfolio runs " + str(accumulated_time/timelimit) + " seconds\n"
    ...
    if((accumulated_time < timelimit) and ((counter == 1) or ((counter > 1) and (not optimal_planning)))):
        ...
        time_aux =  int((timelimit - accumulated_time))
        timeouts = [time_aux, time_aux, time_aux, time_aux]
        original_data = True
        ...
        if((accumulated_time < timelimit) and ((counter == 1) or ((counter > 1) and (not optimal_planning)))):
            planners2 =  ["lama-2011", "lamar", "arvand", "mercury"]
            time_total = []
            for i in range(len(planners2)):
        	    timeFinal = int(math.ceil(timelimit - accumulated_time))
        	    time_total.append(timeFinal)
            print "*** Planner List ***\n"
