...
## Lama-first with input PDDL defined cost
ALIASES["lama-first-cost"] = [
    "--heuristic",
    "hlm,hff=lm_ff_syn(lm_rhw(reasonable_orders=true))",
    "--search", """lazy_greedy([hff,hlm],preferred=[hff,hlm],reopen_closed=false)"""]

## Second iteration and on with a cost bound given by a previous call
ALIASES["lama-second"] = [
    "--if-unit-cost",
    "--heuristic",
    "hlm,hff=lm_ff_syn(lm_rhw(reasonable_orders=true))",
    "--search", """iterated([
                     ...
                     lazy_wastar([hff,hlm],preferred=[hff,hlm],w=2),
                     lazy_wastar([hff,hlm],preferred=[hff,hlm],w=1)
                     ],repeat_last=true,continue_on_fail=true,bound=$1)""",
    "--if-non-unit-cost",
    "--heuristic",
    "hlm,hff=lm_ff_syn(lm_rhw(reasonable_orders=true,"
    "                         lm_cost_type=plusone),transform=adapt_costs(plusone))",
...
ALIASES["seq-opt-hmax"] = [
    "--search", "astar(hmax())"]

ALIASES["seq-opt-ipdb"] = [
    "--search", "astar(ipdb(max_time=$1))"]
