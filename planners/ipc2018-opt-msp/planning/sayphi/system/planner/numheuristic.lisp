...
(defun all-last-facts nil
  (let ((layer (num-layers-graph))
	(all-facts nil))
    (maphash (lambda (key fact)
	       (declare (ignore key))
	       (let ((layer-fact (layer-membership (fact-predpos fact))))
		 (if (and layer-fact (>= layer layer-fact))
		     (push (fact-literal fact) all-facts))))
	     *facts*)
    all-facts))
