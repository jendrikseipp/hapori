...
def main():
    ...
    with timers.timing("Parsing", True):
        task = pddl_parser.parse(task_filename=options.task, domain_filename=options.domain)
        
    if options.writeout_reformulation_logic:
        print('-----------------------------------------------------------------')
        print('Running Baggy') 
        print('Domain:', task.domain_name) 
        print('Problem:', task.task_name) 
        if options.enable_pddl_repair:
            print('PDDL repair has been enabled.') 
        else:
            print('PDDL repair has NOT been enabled.')
        if options.ground_operators:
            print('Operators will be pregrounded.') 
        else:
            print('Operators will NOT be pregrounded.')
        if options.dont_bag != "":
            print('The following types will not be bagged:', options.dont_bag)
        if options.direction == 'fwd':
            print('Bagging will only take place in the forward direction.')
        elif options.direction == 'bck':
            print('Bagging will only take place in the backward direction.')
        else:
            print('Bagging will take place in both directions.')
        print('-----------------------------------------------------------------')
            
           
    # Solution translation
    if options.solution and options.mappings:
        if options.writeout_reformulation_logic:
            print('----------------------TRANSLATING SOLUTION--------------------------') 
        reformulation.translate_solution(task, options.solution, options.mappings)
        if options.writeout_reformulation_logic:
            print('--------------------------------------------------------------------') 
        return
    
    
    if options.from_to_count != "":
        reformulation.from_to_count.import_counts(task, options.from_to_count)
    
    # Make modifications to the domain, and print the new one
    reformulation.task_modifier.modify_task(task)
    if options.enable_pddl_repair:
        reformulation.pddl_repair.repair_domain(task)
    if options.writeout_reformulation_logic:
        print('Rewiring PDDL domain...')
    task_filename, domain_filename = pddl_parser.writeout_reformulated_pddl(task, original = True)
    task = pddl_parser.parse(task_filename, domain_filename) 
    ...
    task = reformulation.transform(task, reachable_action_params)
    
    if options.writeout_reformulated_pddl:
        pddl_parser.writeout_reformulated_pddl(task)
