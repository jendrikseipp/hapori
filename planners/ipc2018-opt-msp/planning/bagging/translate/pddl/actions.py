...
class Action(object):
    ...
    # returns PDDL string representation
    def pddl_str(self):
        result = '(:action %s\n' % self.name

        if self.parameters != []:
            result += '\t:parameters (' + ' '.join([param.pddl_str() for param in self.parameters]) + ')\n'
            # for param in self.parameters:
            # result += "  %s\n" % self.parameters.pddl_str()

        if self.precondition.parts != [] or isinstance(self.precondition, conditions.Atom):
            result += '\t:precondition ' + self.precondition.pddl_str() + '\n'


        if self.effects or True:
            result += '\t:effect (and\n\t'
            result += '\n\t'.join([eff.pddl_str() for eff in self.effects]) + '\n'
            if self.cost:
                result += '\t  ' + self.cost.pddl_str()
            result += ')'
            result += ')\n\n'
        return result
