...
class Effect(object):
    ...
    def pddl_str(self):
        ...
        result = ''
        if self.parameters:
            result += "%s(forall (%s)" % (indent, ", ".join([param.pddl_str() for param in self.parameters]))
            indent += "  "
        if self.condition != conditions.Truth():
            result += "\n%s(when" % ("\t" + indent)
            result += self.condition.pddl_str()
            result += "\n\t"
            indent += "  "
        result += "%s%s" % (indent, self.literal.pddl_str())
        if self.parameters:
             result += "\n\t  ))"
        return result
    def copy(self):
...
class ConditionalEffect(object):
    ...
    def pddl_str(self):
        return 'if ' + self.condition.pddl_str() + '\nthen ' + self.effect.pddl_str() + '\n'
    def normalize(self):
...
class UniversalEffect(object):
    ...
    def dump(self, indent="  "):
        ...
        self.effect.dump(indent + "  ")
    def pddl_str(self):
        return '  forall ' + ' '.join([param.pddl_str() for param in self.parameters]) + '\n   ' + self.effect.pddl_str()
    def normalize(self):
        norm_effect = self.effect.normalize()
        if isinstance(norm_effect, ConjunctiveEffect):
            new_effects = []
...
class ConjunctiveEffect(object):
    ...
    def dump(self, indent="  "):
        ...
        for eff in self.effects:
            eff.dump(indent + "  ")
    def pddl_str(self):
        result = '   and ' + '\n  '.join([eff.pddl_str() for eff in self.effects])
    def normalize(self):
        new_effects = []
