...
class PrimitiveNumericExpression(FunctionalExpression):
    ...
    def pddl_str(self):
        return '(%s %s)' % (self.symbol, " ".join(map(str, self.args)))
    def dump(self, indent="  "):
        print("%s%s" % (indent, self._dump()))
...
class Assign(FunctionAssignment):
    ...
    def pddl_str(self):
        return "(= %s %s)" % (self.fluent.pddl_str(), self.expression.pddl_str())

class Increase(FunctionAssignment):
    def pddl_str(self):
        return '(increase %s %s)' % (self.fluent.pddl_str(), self.expression.pddl_str())
