...
void generate_cpp_input(bool /*solvable_in_poly_time*/,
    ...
    outfile << "end_SG" << endl;
    vector<simplyDTG> final;
    for (int i = 0; i < var_count; i++) {
        outfile << "begin_DTG" << endl;
        //Isa  transition_graphs[i].generate_cpp_input(outfile);
        // transition_graphs[i].generate_cpp_input(outfile)
       	vector<simplyDTG> simp = transition_graphs[i].generate_cpp_input(outfile, store,i);
   	for (int f = 0; f < simp.size(); f++){
   		simplyDTG sim = simp[f];
      	final.push_back(sim);
       	}
        //Isa
        simp.clear();
    ...
    store.setDomainTransitionGraph(final);
    outfile << "begin_CG" << endl;
    //Isa cg.generate_cpp_input(outfile, ordered_vars);
    vector<simplyCG> sp = cg.generate_cpp_input(outfile, ordered_vars, store);
    store.setCausalGraph(sp);
    Features fea = Features();
    fea.calculateValueCG(store);
    fea.calculateValueDTG(store);
    fea.dump();
    fea.dumpFile(outfilefeatures);
    fea.dumpFileSimply(outfilefeatures);
    fea.dumpSimply();
    //Isa
    outfile << "end_CG" << endl;
    //cout<<"len CG:" <<store.getCausalGraph().size()<<endl;
    //cout<<"len DTG:" <<store.getDomainTransitionGraph().size()<<endl;
    store.printFile(outfilefeatures);
    outfile.close();
}
