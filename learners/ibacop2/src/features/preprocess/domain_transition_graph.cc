...
//ISA modified Store store
//void DomainTransitionGraph::generate_cpp_input(ofstream &outfile)
vector<simplyDTG> DomainTransitionGraph::generate_cpp_input(ofstream &outfile, Store store, int variable) const {
    ...
    for (int i = 0; i < vertices.size(); i++) {
        ...
        for (int j = 0; j < vertices[i].size(); j++) {
            ...
            outfile << number << endl;
            //ISA
            simplyDTG simpltytg = simplyDTG(variable, vertices[i].size(), i, trans.target, number+1);
            store.addSimplyDTG(simpltytg);
            //ISA
            for (int k = 0; k < trans.condition.size(); k++)
                if (trans.condition[k].first->get_level() != -1)
    ...
    return store.getDomainTransitionGraph();
}
