...

{
  ...
  if ( ggoal->connective == TRU ) {
    if (gcmd_line.steps_empty_goal > 0)
      printf("\nwarning: goal can be simplified to TRUE. The empty plan solves it\n\n");
    else
      {
  ...
  default:
    if (gcmd_line.steps_empty_goal > 0)
      {
	printf("\n\nwarning won't get here: non COMP,ATOM,AND,OR in fully simplified goal\n\n");
      }
    else
      {
...

{
  ...
  default:
    if (gcmd_line.steps_empty_goal > 0)
      {
	printf("\n\nwarning won't get here: non ATOM,OR,AND in goal set relevants\n\n");
      }
    else
      {
...

{
  ...
  int i, adr, fl;
  FluentValues *fvs;
  Facts *current_static_literal = NULL;
  ...
  for ( f = ginitial; f; f = f->next ) {
    ...
    if ( !lneg[lp][adr] ) {/* non deleted ini */
      /* include the fact as static LITERAL */
      if (current_static_literal == NULL)
	{
	  gStatic_literals = new_Facts();
          gStatic_literals->fact = f->fact; 
	  gStatic_literals->next = NULL;
	  current_static_literal = gStatic_literals;
	}
      current_static_literal->next = new_Facts();
      current_static_literal->next->fact = f->fact; 
      current_static_literal= current_static_literal->next;
...

{
  ...
  for ( i = 0; i < gnum_op_conn; i++ ) {
    ...
    /* raquel 
     */
    gop_conn[i].RP_deactivated = FALSE;
  }
