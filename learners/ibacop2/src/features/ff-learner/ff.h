...
/* size of goals_at array in 1P extraction 25
 */
...
/* max number of lnf effects in one action effect 50
 */
#define MAX_LNF_EFFS 100




/* Parameters for DCK algorithms */
...
#define MAX_PREDICATES 1000 //50
#define MAX_FUNCTIONS 500  //50
#define MAX_TYPES 500  //50
#define MAX_ARITY 50  //5
#define MAX_VARS 300 //15


#define MAX_TYPE 2000


#define MAX_OPERATORS 2000 //200
...
struct _command_line {
  ...
  /* raquel */
  int heuristic;
  Bool use_cost_heuristic;

  int search_algorithm;
  char knowledge_path[MAX_LENGTH];

  Bool stop_at_first_sol;
  int hdiff;
  int roller_bfs_horizon;

  int seed;
  int learner;
  Bool check_repeated;
  Bool save_repeated_lists;
  Bool adm_pruning;
  ...
  int steps_empty_goal;
  int steps_with_goals;
...
typedef struct _Factunion {

  Facts *factlist;
  struct _Factunion *next;
...
typedef struct _OpConn {
  ...
  /* raquel: not active action for building the relaxed planning graph */
  Bool RP_deactivated;
  int level_required;
  int level_achievable;
  Bool deletes_top_level_goal;
...
typedef struct _EfConn {
  ...
  /*raquel: for hlevel computation: acc_PCcost
   */
  float acc_PCcost;
  float acc_cost; /* acc_PCcost + cost */
  int level_applicable; /*level in which an action is applicable*/
...
typedef struct _FtConn {
  ...
  /* raquel: level in which the fact is required */
  int level_required;
  int level_achievable;
  ...
  /* members for computing landmarks */
  Bool is_landmark;
  int *landmark_edges;
  int num_landmark_edges;
  /* deactivated for RP computation. Used for computing landmarks*/
  Bool RP_deactivated;

  /* Tomas: Computing RP fact balance */
  int rp_times_added;
  int rp_times_deleted;
...
typedef struct _FlConn {
  ...
  Bool *def;
  float *level;
  /* raquel: level in which the fluent is required */
  int level_required;
  int level_achievable;
...
/* raquel: Hoffmann BfsNode commented 
 */
/*
...
typedef struct _BfsNode {
  ...
  Bool is_lookahead;
  Bool lookahead_generated;


  struct _BfsNode *children;
  struct _BfsNode *child_next;
  struct _BfsNodeList *repeated_list;
  int num_children;
  Bool tag;
  Bool cm_repeated;
  int number;
  Bool repeated;
  Bool bad_repeated; /* It marks nodes that are in the tree for learning but don't need to be expanded*/
...
/****************************************
 * STRUCTURES FOR HEURISTIC COMPUTATION *
 ****************************************/
/* raquel
 */

typedef struct _OpenApp {
  int ef;
  struct  _OpenApp *next;

} OpenApp;


/*******************************************
//STRUCTURES FOR THE LEARNER
*******************************************/

typedef struct _SolPath{
  BfsNode *node;
  struct _SolPath *next; 
}SolPath;


typedef struct _BfsNodeList{
  BfsNode *node;
  struct _BfsNodeList *next; 
}BfsNodeList;
...
/* helful actions plus heuristic value for the node it generates */
typedef struct _HA_h
{
  int HA;
  int h;
} HA_h;
...
extern int gevaluated_states_lookahead_FF_heuristic;

extern long int ggenerated_nodes;
extern long int ggenerated_atfirst;

/* globlas for learning info */
extern int learn_total_examples;
extern long int learn_positive_transition;
extern long int learn_positive_insolution;
extern long int learn_positive_repeated;

extern long int learn_recog_dead_end;
...
extern int gnum_orig_predicates;
...
extern int *gtarget_goal;
extern int gnum_target_goal;
 ...
 */
extern int g_RP[MAX_PLAN_LENGTH];
extern int gnum_RP;

extern int numero_rp_igual;
extern int numero_rp_distinto;
extern int distinto_misma_h;
extern int distinto_mayor_add;
extern float diff;
/* admissible heuristic related
 */
extern float float_admissible_h;
extern int int_admissible_h;
...
/* Globals for keeping search space for learning*/
extern BfsNode *gSearch_tree;
extern Solution *Sols_for_learning;
extern int numSolutions;
extern int numSols_for_learning;

extern Bool domain_with_static_facts;

/*Globals for computing initial state features*/
extern int feature_hmax, feature_hff;
extern float feature_hff_ratio;
extern int feature_hmax_momentum;

extern int feature_rp_balance_min, feature_goal_balance_min;
extern float feature_rp_balance_avg, feature_rp_balance_var, feature_goal_balance_avg, feature_goal_balance_var;
extern float feature_balance_ratio, feature_unbalance_ratio;
extern unsigned long feature_balance_distorsion;

// This holds the literals that don't belong to a static-predicate
// but are fixed in the initial state.
extern Facts *gStatic_literals;
