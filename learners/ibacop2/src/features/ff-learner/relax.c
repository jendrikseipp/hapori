...
/* helpful actions numeshrical helpers
 */
Comparator *lHcomp;
float *lHc;
...

{
  ...
  /* trace relaxed graphplan */
  if (gcmd_line.search_algorithm == 28)
    features_from_relaxed_graphplan();
...
Bool build_fixpoint(State *S, int *max)
{
  ...
  case H_LEVEL_ADD: result=build_fixpoint_hlevel_add(S, max); break;
     ...
     */
  case H_MAX: result=build_fixpoint_hlevel_max(S, max); break; 
    /* h add computed using the RPG generated by hlevel add
     */
  case H_ADD: result=build_fixpoint_hlevel_add(S, max); break;
  case H_ADD_HA: result=build_fixpoint_hlevel_add(S, max); break;
...

{
  ...
  while ( TRUE ) {
    ...
    if ( start_ft == lnum_F && !OpenApp_head->next) {
      if ( fluents_hopeless( time ) ) {
	/* fixpoint, goals not reached
	 */
	*max = time;
    ...
    stop_ft = lnum_F;
    for ( i = start_ft; i < stop_ft; i++ ) {
      activate_ft_hlevel_max( lF[i], time );
    }
...

{
  ...
  while ( TRUE ) {
	...
	*max = time;
    ...
    stop_ft = lnum_F;
    for ( i = start_ft; i < stop_ft; i++ ) {
      activate_ft_hlevel_add( lF[i], time );
    }
...

{
  ...
  if (gft_conn[index].RP_deactivated) return;
  ...
  for ( j = 0; j < gef_conn[index].num_A; j++ ) {
    ...
    */
    if (gft_conn[gef_conn[index].A[j]].level_achievable == INFINITY)
      gft_conn[gef_conn[index].A[j]].level_achievable = time+1;
...

{
  ...
  for ( i = 0; i < lnum_F; i++ ) {
    ...
    gft_conn[lF[i]].level_required = INFINITY;
    gft_conn[lF[i]].level_achievable = INFINITY;
...

{
  ...
  /* raquel: for the max heuristic */
  if (gcmd_line.heuristic == H_MAX)
    {
      lh = 0;
      /* logic goals */
      for ( i = 0; i < gnum_flogic_goal; i++ ) {
  ...
  /* raquel for the add heuristic */
  if (gcmd_line.heuristic == H_ADD) {
    lh = 0;
    gcost = 0;
    /* logic goals */
    for ( i = 0; i < gnum_flogic_goal; i++ ) {
  ...
  /* raquel */
  if (gcmd_line.heuristic == H_LEVEL_MAX ||gcmd_line.heuristic == H_MFF )
    {
      float_admissible_h = level_cost[max_goal_level];
      int_admissible_h = max_goal_level;
    }

  /* raquel for the add heuristic with helpful actions*/
  if (gcmd_line.heuristic == H_ADD_HA) {
    lh = 0;
    gcost = 0;
    /* logic goals */
    for ( i = 0; i < gnum_flogic_goal; i++ ) {
  ...
  */
  ...
  for ( i = 0; i < gnum_real_fl_conn; i++ ) {
	...
	if ( gcmd_line.optimize && goptimization_established ) {
       ...
       */
      select_op( time, gef_conn[ef].op, gfl_conn[i].level_required, gfl_conn[i].level_achievable,0 );
  ...
  for ( i = 0; i < lnum_goals_at[time]; i++ ) {
    ...
    */
    ...
    select_op( time, gef_conn[ef].op, gft_conn[ft].level_required, gft_conn[ft].level_achievable, min_p);
  ...
  if ( gop_conn[op].is_used != time - 1 ) {
    ...
    if ( gcmd_line.display_info == 127 ) {
      ...
      printf(" (cost + gtt %f) (level %d) (level-cost %f)\n", 
	     gef_conn[op].cost + gtt, gef_conn[op].level, level_cost[gef_conn[op].level]);
    }
  }
...

{
  ...
  for ( k = 0; k < gef_conn[ef].num_PC; k++ ) {
    ...
    if (LESS(time,gft_conn[ft].level_required)){
      gft_conn[ft].level_required = time;
    }
...

{
  ...
  /* raquel: for the case of hmax and hadd there aren't helpful actions: no
     ...
     in level 0 */
  if (gcmd_line.heuristic == H_MAX || gcmd_line.heuristic == H_ADD)
    {
      if ( first_call ){
  ...
  if (first_call)
	...
	for (i = 0; i < gef_conn[index].num_D && !next_ef; i++ ){
	  ...
	  for ( k = 0; k < gnum_flogic_goal; k++ ){
		...
		gop_conn[gef_conn[index].op].deletes_top_level_goal = TRUE;
...
void features_from_relaxed_graphplan(void){
  int i,j, level, ft, op;
  int rp_level_actions, rgraph_level_actions;
  int hmax_momentum, diff_hmax;
  int rp_balance_sum, i_rp_balance, rp_balance_square_sum;
  int rp_g_balance_sum, i_rp_g_balance, rp_g_balance_square_sum;
  
  /*hmax_momentum = 0;
  
  printf("\n\n Relaxed Graphplan\n");
  printf("\n Level RGraph RPactions RatioGraph RatioRP");
  
  for(i = 0; i < gnum_flogic_goal; i++){
    ft = gflogic_goal[i]; 
    print_ft_name(ft);
    printf (" %d", gft_conn[ft].level);
    printf("\n");
    
    diff_hmax = int_admissible_h - gft_conn[ft].level;
    hmax_momentum = hmax_momentum + (diff_hmax * diff_hmax);
    
  }
  
  printf ("\n hmax_momentum: %d", hmax_momentum);
  printf ("\n sum_difficulty: %d", sum_difficulty);
  
  feature_hmax = int_admissible_h;
  feature_hmax_momentum = hmax_momentum;
    
  */


  for ( i = 0; i < gnum_relevant_facts; i++ ) {
    gft_conn[i].rp_times_added = 0;
    gft_conn[i].rp_times_deleted = 0;
  }
  ...
  for (i = gnum_RP-1; i>=0; i--)
	...
	gft_conn[gef_conn[op].A[j]].rp_times_added++;
      }
      for (j=0; j < gef_conn[op].num_D; j++){
	gft_conn[gef_conn[op].D[j]].rp_times_deleted++;
      }
    }
  
  
  feature_rp_balance_min = gnum_RP;
  rp_balance_sum = 0;
  rp_balance_square_sum = 0;
  
  for ( i = 0; i < gnum_relevant_facts; i++ ) {
    i_rp_balance = gft_conn[i].rp_times_added - gft_conn[i].rp_times_deleted;
    
    if (i_rp_balance < feature_rp_balance_min)
      feature_rp_balance_min = i_rp_balance;
    
    rp_balance_sum = rp_balance_sum + i_rp_balance;
    rp_balance_square_sum = rp_balance_square_sum + (i_rp_balance * i_rp_balance);
    
    /*printf("\n%d: ", i);
    print_Fact( &(grelevant_facts[i]) );
    printf(" A:%d  D:%d", gft_conn[i].rp_times_added, gft_conn[i].rp_times_deleted);
    */
  }
  
  feature_rp_balance_avg = (float) rp_balance_sum / (float) gnum_relevant_facts;
  feature_rp_balance_var = sqrt(((float) rp_balance_square_sum / (float) gnum_relevant_facts) - (feature_rp_balance_avg * feature_rp_balance_avg));
			
  printf("\n rp_fact_balance_min:  %d", feature_rp_balance_min);
  printf("\n rp_fact_balance_avg:  %2f", feature_rp_balance_avg);
  printf("\n rp_fact_balance_var:  %2f", feature_rp_balance_var);

  /* computing rp fact balance, but only considering goals*/
  feature_goal_balance_min = gnum_RP;
  rp_g_balance_sum = 0;
  rp_g_balance_square_sum = 0;

  for ( i = 0; i < gnum_flogic_goal; i++ ) {
    i_rp_g_balance = gft_conn[gflogic_goal[i]].rp_times_added - gft_conn[gflogic_goal[i]].rp_times_deleted;
    
    if (i_rp_g_balance < feature_goal_balance_min)
      feature_goal_balance_min = i_rp_g_balance;
    
    rp_g_balance_sum = rp_g_balance_sum + i_rp_g_balance;
    rp_g_balance_square_sum = rp_g_balance_square_sum + (i_rp_g_balance * i_rp_g_balance);
    
    /*printf("\n%d: ", i);
    print_Fact( &(grelevant_facts[gflogic_goal[i]]));
    printf(" A:%d  D:%d", gft_conn[gflogic_goal[i]].rp_times_added, gft_conn[gflogic_goal[i]].rp_times_deleted);
    */

  }
  
  feature_goal_balance_avg = (float) rp_g_balance_sum / (float) gnum_flogic_goal;
  feature_goal_balance_var = sqrt(((float) rp_g_balance_square_sum / (float) gnum_flogic_goal) - (feature_goal_balance_avg * feature_goal_balance_avg));
			
  printf("\n rp_goal_balance_min:  %d", feature_goal_balance_min);
  printf("\n rp_goal_balance_avg:  %2f", feature_goal_balance_avg);
  printf("\n rp_goal_balance_var:  %2f", feature_goal_balance_var);

  /*for ( level = 0; level < int_admissible_h; level++){
    rp_level_actions = 0;
    rgraph_level_actions = 0;
    
    for ( i = 0; i < gnum_ef_conn; i++ ) {      
      if ( gef_conn[i].level == level){
	rgraph_level_actions++;
      }
    }
    for (i = gnum_RP-1; i>=0; i--){
      if (gop_conn[g_RP[i]].is_used == level){
	rp_level_actions++;
      }
    }
  

    printf("\n%d, %d, %d", level, rgraph_level_actions, rp_level_actions);
  }
  */

  int *pgf_serie, *rpf_serie, *rpdel_serie, *goal_serie, *act_serie, *unb_serie, *bal_serie;
  int ilevel;
  int **balance_matrix;
  Bool *in_balance;
  int gap, adfact, balhorizon = int_admissible_h + 1;
  int k, balance_target, layersum, balance_gap, dfactor, kdistorsion;
  unsigned long balance_distorsion;
  float iratio, balance_ratio = 0.0, unbalance_ratio = 0.0;
  
  pgf_serie = ( int * ) calloc( int_admissible_h+1, sizeof( int ));
  rpf_serie = ( int * ) calloc( int_admissible_h+1, sizeof( int ));
  rpdel_serie = ( int * ) calloc( int_admissible_h+1, sizeof( int ));
  goal_serie = ( int * ) calloc( int_admissible_h+1, sizeof(int));
  act_serie =   ( int * ) calloc( int_admissible_h+1, sizeof(int));
  unb_serie =   ( int * ) calloc( int_admissible_h+1, sizeof(int));
  bal_serie =   ( int * ) calloc( int_admissible_h+1, sizeof(int));

  printf ("\n");
  for ( i = 0; i < gnum_relevant_facts; i++ ) {
    /*print_ft_name(i);
      printf (" %d", gft_conn[i].level); */
      if (gft_conn[i].level >= 0)
	{
	pgf_serie[gft_conn[i].level]++;
	}
  ...
  for (i = gnum_RP-1; i>=0; i--)
    {
      ...
      //print_op_name( op );
      //printf(" :%d ", gop_conn[op].is_used);
      ilevel = gef_conn[op].level + 1;
      
      for (j=0; j < gef_conn[op].num_A; j++){
	

	/*printf("\n");
	print_ft_name(gef_conn[op].A[j]);
	printf(" added at: %d ", ilevel );*/
	rpf_serie[ilevel]++;
      }
      for (j=0; j < gef_conn[op].num_D; j++){

	/*printf("\n");
	print_ft_name(gef_conn[op].D[j]);
	printf("deleted at: %d ", ilevel );*/
    ...
    for ( i = 0; i < gnum_flogic_goal; i++ ) {
      ilevel =  gft_conn[gflogic_goal[i]].level;
      goal_serie[ilevel]++;
    }
	...
	//printf("\n");
	//print_op_name(i);
	//printf (":%d", gef_conn[i].level);
	ilevel = gef_conn[i].level;
	if (ilevel >= 0){
	  act_serie[ilevel]++;
	}
      }
  ...
  printf ("\n Planning Graph Fact Serie:\t\t");
  for (i=0;  i<= int_admissible_h;  i++)
  {
    printf(" %d", pgf_serie[i]);
  }
    printf ("\n Relaxed Plan Add Serie:\t\t");
  for (i=0;  i<= int_admissible_h;  i++)
  {
    printf(" %d", rpf_serie[i]);
  }
  printf ("\n Relaxed Plan Del Serie:\t\t");
  for (i=0;  i<= int_admissible_h;  i++)
  {
    printf(" %d", rpdel_serie[i]);
  }
  
  printf ("\n Achieved Goal Serie:\t\t\t");
  for (i=0;  i<= int_admissible_h;  i++)
  {
    printf(" %d", goal_serie[i]);
  }

  printf ("\n Applicable Action Serie:\t\t");
  for (i=0;  i<= int_admissible_h;  i++)
  {
    printf(" %d", act_serie[i]);
  }
  

  

  in_balance = (Bool *) calloc (gnum_relevant_facts, sizeof (Bool));
  
  /* +2 becuase the initial state and the virtual final state*/
  balance_matrix = ( int ** ) calloc( balhorizon , sizeof( int* ));
  for (i= 0 ; i < balhorizon; i++)
    balance_matrix[i] = (int *)  calloc ( gnum_relevant_facts, sizeof (int));

  balance_distorsion = 0;

  /* Adding the initial state */
  for(i=0; i < ginitial_state.num_F; i++){
    balance_matrix[0][ginitial_state.F[i]] = 1;
    in_balance[ ginitial_state.F[i] ] = TRUE; 
  }

  
  /* Adding facts from relaxed plan */
  for (i = gnum_RP-1; i>=0; i--)
    {
      op = g_RP[i];
      ilevel = gef_conn[op].level + 1;
	...
	printf(" :%d ", gop_conn[op].is_used);
      */
      for (j=0; j < gef_conn[op].num_A; j++){
	adfact = gef_conn[op].A[j];
	balance_matrix[ilevel][adfact]++;
	if (!in_balance[adfact])
	  in_balance[adfact] = TRUE;
	  
      }
      for (j=0; j < gef_conn[op].num_D; j++){
	adfact = gef_conn[op].D[j];
	balance_matrix[ilevel][adfact]--;
	if (!in_balance[adfact])
	  in_balance[adfact] = TRUE;
      }
  ...
  for (j = 0; j < gnum_relevant_facts; j++){
    ...
    layersum = 0;
    for (i = 0; i < balhorizon; i++){
      layersum = layersum + balance_matrix[i][j];

      if (balance_matrix[i][j] < 0 && layersum < balance_target){
	/* compute the distorsion until achieving the balance target again */
	unb_serie[i] = unb_serie[i] + (balance_target - layersum);

	balance_gap = layersum;
	for (k = i+1, gap = 1; k < balhorizon; k++, gap++){
	  balance_gap = balance_gap + balance_matrix[k][j];
	  if (balance_gap >= balance_target)
	    break;
	  
	}
	dfactor = 1 << gap;
	kdistorsion =  (balance_target - layersum) * gap;
	balance_distorsion += kdistorsion;
	//printf("\n   adding %d to distorsion of", kdistorsion);
	//print_ft_name(j);
      }
      else if (balance_matrix[i][j] != 0 && layersum >= balance_target){
	bal_serie[i]++;
      }

    }
   
    /*if (layersum < balance_target){
      printf("\n **");
      print_ft_name(j);
  ...
  printf ("\n Unbalance Fact Serie:\t\t\t");
  for (i=0;  i<= int_admissible_h;  i++)
  {
    printf(" %d", unb_serie[i]);
  }

  printf ("\n Balance Fact Serie:\t\t\t");
  for (i=0;  i<= int_admissible_h;  i++)
  {
    printf(" %d", bal_serie[i]);
  }
  
  printf ("\n\n Num relevant ops: %d", gnum_ef_conn);
  
  for (i=1;  i< int_admissible_h;  i++){
    if (unb_serie[i] > 0){
      
      iratio = ((float) act_serie[i] * (float) unb_serie[i] / (float) gnum_ef_conn);
      //printf("....>> %.2f", iratio);
      unbalance_ratio = unbalance_ratio + iratio;
    }
    
    if (bal_serie[i] > 0){
      iratio = ((float) act_serie[i] * (float) bal_serie[i] / (float) gnum_ef_conn);
      //printf("....>> %.2f", iratio);
      balance_ratio = balance_ratio + iratio;
    }


  }
  
  printf("\nbalance Ratio: %.3f", balance_ratio);
  printf("\nUnbalance Ratio: %.3f", unbalance_ratio);
  printf("\nBalance Distorsion: %lu", balance_distorsion);

  feature_balance_ratio = balance_ratio;
  feature_unbalance_ratio = unbalance_ratio;
  feature_balance_distorsion = balance_distorsion;
  
  printf("\nBalance Distorsion: %lu", feature_balance_distorsion);
  /*
  printf("\n\n --- BALANCE MATRIX ---");
  for (j=0; j< gnum_relevant_facts; j++){
    if (in_balance[j]){
