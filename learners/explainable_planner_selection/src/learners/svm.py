...
class SVM(SKLearnPerPlannerML):
    def __init__(self, kernel, **kwargs):
        super().__init__(**kwargs)
        self.kernel = kernel
        assert self._label_transformation in [LabelTransformations.BINARY]
    ...
    def train(self, x_train, y_train, x_valid, y_valid, **kwargs):
        ...
        for i in range(y_train.shape[1]):
            ...
            self._model.append(svm.SVC(kernel=self.kernel))
...
class SVMFactory(BaseMLFactory):
    def parse(self, args):
        return parser.parse_args(args)

    def setup(self, main_options, learner_options):
        return SVM(learner_options.kernel,
