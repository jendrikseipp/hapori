...
class ClusterPortfolio(Portfolio):
    ...
    def compute_portfolio(self):
        ...
        for config_id, config in enumerate(self.configs):
            ...
            print config_id, config.split('-')[1].ljust(22), group_id, distances[config_id]
        ...
        for group_id, cluster in sorted(clusters.items()):
            ...
            print group_id, str(len(cluster)).rjust(2),
            print ', '.join(config.split('-')[1] for config in cluster),
            print '->', best_config.split('-')[1]

        for config in representatives:
            config_id = self.configs.index(config)
